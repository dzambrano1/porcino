...
if ($action === 'upload') {
    ...
    if (curl_errno($curl)) {
        ...
        logDebug("Upload CURL Error Details", ['errno' => $curlErrno, 'error' => $curlError]);
        
        // Provide more specific error messages based on cURL error codes
        $errorMessage = 'Curl error: ' . $curlError . ' (Code: ' . $curlErrno . ')';
        if ($curlErrno === CURLE_COULDNT_RESOLVE_HOST) {
            $errorMessage = 'No se pudo conectar al servidor de ChatPDF. Verifique su conexión a internet.';
            ...
            $errorMessage = 'No se pudo establecer conexión con el servidor. Intente nuevamente.';
        }
    ...
    logDebug("Raw Input Data", ['raw_input' => $rawInput, 'content_length' => strlen($rawInput)]);
    
    $input = json_decode($rawInput, true);
    
    if (!$input || !isset($input['sourceId']) || empty($input['sourceId']) || !isset($input['messages'])) {
        logDebug("Invalid Request Data", [
            'input' => $input,
            'raw_input' => $rawInput,
            'json_error' => json_last_error_msg(),
            'sourceId_empty' => empty($input['sourceId'] ?? ''),
            'sourceId_null' => is_null($input['sourceId'] ?? null)
        ]);
        sendError('Invalid request data: sourceId is required and cannot be empty');
    }
    ...
    if (curl_errno($curl)) {
        ...
        $curlError = curl_error($curl);
        $curlErrno = curl_errno($curl);
        logDebug("CURL Error Details", ['errno' => $curlErrno, 'error' => $curlError]);
