...
try {
    ...
    if (!isset($conn)) {
        logError('Connection not initialized after including pdo_conexion.php');
        $debugInfo['error'] = 'Connection not initialized';
        ...
        if ($row_count > 0) {
            ...
            $monthlyQuery = "
                SELECT 
                    ...
                    AVG(ph_peso_precio) as avg_price,
                    COUNT(DISTINCT ph_peso_tagid) as animal_count
                FROM ph_peso
                GROUP BY month
                ORDER BY month ASC
                LIMIT 12";
            ...
            try {
                ...
                if (!empty($monthlyData)) {
                    ...
                    $monthlyMilkQuery = "
                        WITH RankedSamples AS (
                            SELECT 
                                ...
                                ph_leche_peso,
                                ph_leche_precio,
                                LEAD(ph_leche_fecha) OVER (PARTITION BY ph_leche_tagid ORDER BY ph_leche_fecha) as next_fecha
                            FROM ph_leche
                        ),
                        ProductionCalculations AS (
                            SELECT 
                                ...
                                ph_leche_precio,
                                COALESCE(
                                    DATEDIFF(next_fecha, ph_leche_fecha),
                                    DATEDIFF(CURRENT_DATE, ph_leche_fecha)
                                ) as days_until_next,
                                ph_leche_peso * 
                                COALESCE(
                                    DATEDIFF(next_fecha, ph_leche_fecha),
                        ...
                        SELECT 
                            ...
                            AVG(ph_leche_precio) as avg_price,
                            COUNT(DISTINCT ph_leche_tagid) as animal_count
                    ...
                    try {
                        $stmt = $conn->prepare($monthlyMilkQuery);
                        $stmt->execute();
