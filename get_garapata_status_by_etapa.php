...
try {
    ...
    $sql_counts = "
        SELECT
            ...
            COUNT(DISTINCT a.ph_garrapatas_tagid) AS vaccinated_animals
        FROM porcino p
        LEFT JOIN ph_garrapatas a ON p.tagid = a.ph_garrapatas_tagid
        WHERE p.etapa IN ('Inicio', 'Crecimiento', 'Finalizacion')
        GROUP BY p.etapa;
    ";
    ...
    $sql_list = "
        ...
        LEFT JOIN ph_garrapatas a ON p.tagid = a.ph_garrapatas_tagid
        WHERE p.etapa IN ('Inicio', 'Crecimiento', 'Finalizacion') 
          AND a.ph_garrapatas_tagid IS NULL
        ORDER BY p.etapa, p.nombre;
    ";
