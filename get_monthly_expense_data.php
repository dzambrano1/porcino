...
try {
    ...
    $sql = "
        ...
        FROM ph_concentrado
        WHERE ph_concentrado_racion > 0 AND ph_concentrado_costo > 0 -- Consider only valid cost entries
        ORDER BY ph_concentrado_tagid, ph_concentrado_fecha_inicio ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['ph_concentrado_tagid'];
            $fecha = $row['ph_concentrado_fecha_inicio'];
            $daily_cost = (float)$row['ph_concentrado_racion'] * (float)$row['ph_concentrado_costo'];
