...
try {
    ...
    $sql = "
        SELECT
            ...
            ph_melaza_costo
        FROM ph_melaza
        WHERE ph_melaza_racion > 0 AND ph_melaza_costo > 0 -- Consider only valid cost entries
        ORDER BY ph_melaza_tagid, ph_melaza_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['ph_melaza_tagid'];
            $fecha = $row['ph_melaza_fecha'];
            $daily_cost = (float)$row['ph_melaza_racion'] * (float)$row['ph_melaza_costo'];
