...
try {
    ...
    $sql = "
        SELECT
            ...
            ph_sal_costo
        FROM ph_sal
        WHERE ph_sal_racion > 0 AND ph_sal_costo > 0 -- Consider only valid cost entries
        ORDER BY ph_sal_tagid, ph_sal_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['ph_sal_tagid'];
            $fecha = $row['ph_sal_fecha'];
            $daily_cost = (float)$row['ph_sal_racion'] * (float)$row['ph_sal_costo'];
