...
try {
    ...
    $sql_counts = "
        SELECT
            ...
            COUNT(DISTINCT a.ph_neumonia_tagid) AS vaccinated_animals
        FROM porcino p
        LEFT JOIN ph_neumonia a ON p.tagid = a.ph_neumonia_tagid
    ...
    $sql_list = "
        ...
        LEFT JOIN ph_neumonia a ON p.tagid = a.ph_neumonia_tagid
        WHERE p.grupo IS NOT NULL AND p.grupo != ''
          AND a.ph_neumonia_tagid IS NULL
    ...
    error_log("Error fetching Neumonia status by grupo: " . $e->getMessage());
