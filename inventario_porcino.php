...
$sql = "SELECT id, tagid, nombre, genero, raza, etapa, grupo, estatus, fecha_nacimiento, image, image2, image3, video 
        FROM porcino" . $where_clause . " ORDER BY tagid ASC";
...
// Sexo Counts
$sexoCountsQuery = "SELECT genero, COUNT(*) as count FROM porcino" . $where_clause . " GROUP BY genero";
...
// Raza Counts
$razaCountsQuery = "SELECT raza, COUNT(*) as count FROM porcino" . $where_clause . " GROUP BY raza";
...
// Etapa Counts
$etapaCountsQuery = "SELECT etapa, COUNT(*) as count FROM porcino" . $where_clause . " GROUP BY etapa";
...
// Grupos Counts
$grupoCountsQuery = "SELECT grupo, COUNT(*) as count FROM porcino" . $where_clause . " GROUP BY grupo";
...
// Estatus Counts
$estatusCountsQuery = "SELECT estatus, COUNT(*) as count FROM porcino" . $where_clause . " GROUP BY estatus";
...
$tagidsQuery = "SELECT tagid FROM porcino" . $where_clause;
...
$avgPesoQuery = "
    SELECT 
        DATE_FORMAT(ph_peso_fecha, '%Y-%m') AS peso_month, 
        AVG(ph_peso_animal) AS average_weight 
    FROM 
        ph_peso 
    WHERE 
        ph_peso_tagid IN (" . str_repeat('?,', count($tagids) - 1) . "?) 
    GROUP BY 
        peso_month 
    ORDER BY 
        peso_month ASC
...
// Fetch average monthly ph_concentrado_racion * ph_concentrado_costo from ph_concentrado table
$avgRacionQuery = "
    SELECT 
        DATE_FORMAT(ph_concentrado_fecha_inicio, '%Y-%m') AS racion_month,
        AVG(ph_concentrado_racion * ph_concentrado_costo) AS average_racion_cost
    FROM 
        ph_concentrado
    GROUP BY 
        racion_month
...
$tagidQuery = "SELECT DISTINCT ph_parto_tagid FROM ph_parto";
...
$partoQuery = "
     SELECT 
         DATE_FORMAT(ph_parto_fecha, '%Y-%m') AS yearmonth,
         ph_parto_tagid,
         SUM(ph_parto_numero) AS total_parto
     FROM 
         ph_parto
     GROUP BY 
         DATE_FORMAT(ph_parto_fecha, '%Y-%m'), ph_parto_tagid
     ORDER BY 
         DATE_FORMAT(ph_parto_fecha, '%Y-%m') ASC
";
...
<head>
    ...
    <title>Inventario Porcino</title>
    <!-- Link to the Favicon -->
...
<style>
    ...
    /* Full Screen Chat Styles - ChatGPT Dark Theme */
    .chat-fullscreen {
        position: fixed;
        top: 0;
        left: 0;
    ...
    .chat-input-container #userInput::placeholder {
        color: #b0b0b0;
    }
    ...
    /* Floating Prompt Menu Styles - Dark Theme */
...
<div class="container nav-icons-container">
    <div class="icon-button-container">
        <button onclick="window.location.href='./porcino_register_compras.php'" class="icon-button">
            <img src="./images/venta-metodo-de-pago.png" alt="Compras" class="nav-icon">
        </button>
        <span class="button-label">COMPRAS</span>
    </div>

    <div class="icon-button-container">
        <button onclick="window.location.href='./porcino_register_nacimientos.php'" class="icon-button">
            <img src="./images/parto.png" alt="Nacimientos" class="nav-icon">
        </button>
...
<div id="chatFullscreen" class="chat-fullscreen hidden">
    ...
    <div id="floatingPromptMenu" class="floating-prompt-menu hidden">
        ...
        <div class="floating-menu-content">
            ...
            <p style="margin-top: 15px; font-size: 0.9em; color: #b0b0b0; text-align: center;">
...
<script>
    ...
    function registrar(tagid) {
        if (confirm('¿Desea generar un informe completo para este animal?')) {
            ...
            window.location.href = 'porcino_report.php?tagid=' + encodeURIComponent(tagid);
        }
    }
</script>
<script>
    ...
    function openUpdateModal(tagid) {
        ...
        console.log('Starting AJAX request to fetch_porcino_data.php');
        
        $.ajax({
            url: 'fetch_porcino_data.php',
            type: 'GET',
            data: { tagid: tagid },
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="etapa" id="updateEtapa" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_etapa = "SELECT DISTINCT etapa FROM porcino ORDER BY etapa";
...
async function generateAnimalPDF(tagid) {
    try {
        ...
        const response = await fetch(`porcino_report.php?tagid=${encodeURIComponent(tagid)}`, {
            headers: {
...
async function generateAndUploadPDF(tagid) {
    if (!tagid) {
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: 'No se proporcionó el ID del animal',
    ...
    Swal.fire({
        title: 'Generando Reporte Veterinario',
        html: '<i class="fas fa-file-pdf"></i> Generando PDF del historial médico...',
    ...
    try {
        const pdfResponse = await fetch(`porcino_report.php?tagid=${encodeURIComponent(tagid)}`, {
        ...
        if (!pdfData.success || !pdfData.filename) {
            throw new Error(pdfData.message || 'No se pudo generar el PDF');
        }

        const filename = pdfData.filename;

        Swal.update({ html: '<i class="fas fa-cloud-upload-alt"></i> Subiendo a ChatPDF IA...' });

        const fileResponse = await fetch(`reports/${filename}`);
        if (!fileResponse.ok) throw new Error('Error accediendo al PDF');
        ...
        if (!uploadResponse.ok) {
            throw new Error(uploadData.error || 'Error al subir el PDF');
        }

        Swal.close();
        openChatPDFModal(uploadData.sourceId, filename, tagid);
        ...
        Swal.fire({
            icon: 'error',
            title: 'Error',
            text: error.message || 'No se pudo generar o subir el reporte',
            confirmButtonText: 'Cerrar'
        });
    }
}
...
function openChatPDFModal(sourceId, filename, tagid) {
    ...
    modal.innerHTML = `
        <div class="chatpdf-container" style="width: 90%; max-width: 1200px; height: 85vh; background: #343541; border-radius: 12px; display: flex; flex-direction: column; box-shadow: 0 25px 50px rgba(0,0,0,0.5); overflow: hidden; animation: slideUp 0.3s;">
            ...
            <div style="background: #40414f; padding: 16px 24px; border-top: 1px solid #565869;">
                <div style="max-width: 1000px; margin: 0 auto 12px auto; display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px;">
                    <button class="chatpdf-quick-btn" data-question="¿Cuál es el estado de salud actual de este animal?" style="background: #343541; border: 1px solid #565869; color: #ececf1; padding: 10px 14px; border-radius: 20px; cursor: pointer; font-size: 13px; transition: all 0.2s; white-space: nowrap; text-align: center;">
                        <i class="fas fa-heartbeat"></i> Estado de Salud
                    </button>
                    <button class="chatpdf-quick-btn" data-question="¿Qué vacunas y tratamientos ha recibido?" style="background: #343541; border: 1px solid #565869; color: #ececf1; padding: 10px 14px; border-radius: 20px; cursor: pointer; font-size: 13px; transition: all 0.2s; white-space: nowrap; text-align: center;">
                        <i class="fas fa-syringe"></i> Vacunas
                    </button>
                    <button class="chatpdf-quick-btn" data-question="¿Cómo está su producción y peso?" style="background: #343541; border: 1px solid #565869; color: #ececf1; padding: 10px 14px; border-radius: 20px; cursor: pointer; font-size: 13px; transition: all 0.2s; white-space: nowrap; text-align: center;">
                        <i class="fas fa-chart-line"></i> Producción
                    </button>
                    <button class="chatpdf-quick-btn" data-question="Dame recomendaciones veterinarias para este animal" style="background: #343541; border: 1px solid #565869; color: #ececf1; padding: 10px 14px; border-radius: 20px; cursor: pointer; font-size: 13px; transition: all 0.2s; white-space: nowrap; text-align: center;">
                        <i class="fas fa-clipboard-check"></i> Recomendaciones
                    </button>
                </div>
                <style>
                    @media (max-width: 992px) {
                        .chatpdf-quick-btn:nth-child(1) { grid-column: 1 / -1; }
                        .chatpdf-quick-btn:nth-child(2) { grid-column: 1 / -1; }
                        .chatpdf-quick-btn:nth-child(3) { grid-column: 1 / -1; }
                        .chatpdf-quick-btn:nth-child(4) { grid-column: 1 / -1; }
                        .chatpdf-quick-btn:parent { grid-template-columns: repeat(2, 1fr) !important; }
                    }
                    @media (max-width: 768px) {
