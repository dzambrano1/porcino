...
if (!empty($fetched_orders)) {
    ...
    $items_query = "SELECT od.*, v.image AS porcino_image -- Use alias for porcino image 
                    FROM order_details od 
                    LEFT JOIN porcino v ON od.tagid = v.tagid 
    ...
    foreach ($fetched_orders as $order) {
        ...
        try {
            ...
            foreach ($items_result as $item) {
                 // Prefer order_details image if exists, otherwise use porcino image
                $item['image'] = $item['image'] ?: ($item['porcino_image'] ?: './images/default_animal.png');
                unset($item['porcino_image']); // Clean up alias
                $order_details[] = $item;
            }
