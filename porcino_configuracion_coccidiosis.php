...
<title>Porcino Configuracion Coccidiosis</title>
<!-- Link to the Favicon -->
    ...
    <div class="container-fluid">
        <div class="row w-100">
            <div class="col-12 d-flex justify-content-between align-items-center position-relative">
                <!-- Botón de Volver -->
                <button type="button" onclick="window.location.href='./inventario_porcino.php'" class="btn" style="color:white; border: none; border-radius: 8px; padding: 8px 15px; z-index: 1050; position: relative;" title="Volver al Paso 1">
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando Coccidiosis</span>
...
<!-- New Entry Modal Configuracion Coccidiosis -->

<!-- Add New Vacuna Coccidiosis Button -->
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Coccidiosis
              </h5>
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="coccidiosisTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $coccidiosisQuery = "SELECT * FROM pc_coccidiosis";

                      $stmt = $conn->prepare($coccidiosisQuery);
                      $stmt->execute();
                      $coccidiosissData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($coccidiosissData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($coccidiosissData as $row) {
                              echo "<tr>";
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-coccidiosis" 
                                              ...
                                              data-vacuna="' . htmlspecialchars($row['pc_coccidiosis_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['pc_coccidiosis_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['pc_coccidiosis_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['pc_coccidiosis_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Coccidiosis">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-coccidiosis" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Coccidiosis">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['pc_coccidiosis_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['pc_coccidiosis_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['pc_coccidiosis_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['pc_coccidiosis_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewCoccidiosis').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_coccidiosis.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-coccidiosis').click(function() {
        ...
        // Edit Configuracion Coccidiosis Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editCoccidiosisModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Coccidiosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditCoccidiosis').click(function() {
            ...
            var form = document.getElementById('editCoccidiosisForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de coccidiosis?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-coccidiosis').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de coccidiosis? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
