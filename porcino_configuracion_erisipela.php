...
<title>Porcino Configuracion Erisipela</title>
<!-- Link to the Favicon -->
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando Erisipela</span>
...
<!-- New Entry Modal Configuracion Erisipela -->

<!-- Add New Vacuna Erisipela Button -->
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Erisipela
              </h5>
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="erisipelaTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $erisipelaQuery = "SELECT * FROM pc_erisipela";
                      ...
                      if (empty($erisipelasData)) {
                          ...
                          foreach ($erisipelasData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-erisipela" 
                                              ...
                                              data-vacuna="' . htmlspecialchars($row['pc_erisipela_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['pc_erisipela_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['pc_erisipela_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['pc_erisipela_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Erisipela">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-erisipela" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Erisipela">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['pc_erisipela_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['pc_erisipela_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['pc_erisipela_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['pc_erisipela_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewErisipela').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_erisipela.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-erisipela').click(function() {
        ...
        // Edit Configuracion Erisipela Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editErisipelaModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Erisipela
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditErisipela').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de erisipela?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-erisipela').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de erisipela? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
