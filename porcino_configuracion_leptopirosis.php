...
<title>Porcino Configuracion Leptopirosis</title>
<!-- Link to the Favicon -->
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando Leptopirosis</span>
...
<!-- New Entry Modal Configuracion Leptopirosis -->

<!-- Add New Vacuna Leptopirosis Button -->
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Leptopirosis
              </h5>
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="leptopirosisTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $leptopirosisQuery = "SELECT * FROM pc_leptopirosis";
                      ...
                      if (empty($leptopirosissData)) {
                          ...
                          foreach ($leptopirosissData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-leptopirosis" 
                                              ...
                                              data-vacuna="' . htmlspecialchars($row['pc_leptopirosis_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['pc_leptopirosis_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['pc_leptopirosis_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['pc_leptopirosis_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Leptopirosis">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-leptopirosis" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Leptopirosis">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['pc_leptopirosis_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['pc_leptopirosis_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['pc_leptopirosis_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['pc_leptopirosis_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewLeptopirosis').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_leptopirosis.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-leptopirosis').click(function() {
        ...
        // Edit Configuracion Leptopirosis Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editLeptopirosisModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Leptopirosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditLeptopirosis').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de leptopirosis?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-leptopirosis').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de leptopirosis? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
