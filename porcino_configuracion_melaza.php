...
<title>Porcino Configuracion Melaza</title>
<!-- Link to the Favicon -->
                      ...
                      <div class="input-group">
                          <span class="input-group-text">
                              ...
                              <select class="form-select" id="new_melaza" name="melaza" required>
                                  ...
                                  <?php
                                  $sql_names = "SELECT DISTINCT pc_melaza_nombre FROM pc_melaza ORDER BY pc_melaza_nombre ASC";
                                  $stmt_names = $conn->prepare($sql_names);
                                  $stmt_names->execute();
                                  $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                  foreach ($names as $name_row) {
                                      echo '<option value="' . htmlspecialchars($name_row['pc_melaza_nombre']) . '">' . htmlspecialchars($name_row['pc_melaza_nombre']) . '</option>';
                                  }
                                  ?>
                              </select>
                          </span>                            
                      ...
                      <div class="input-group">
                          <span class="input-group-text">
                              ...
                              <select class="form-select" id="new_etapa" name="etapa" required>
                                  <option value="">Seleccionar</option>
                                  <?php
                                  $sql_etapas = "SELECT DISTINCT pc_melaza_etapa FROM pc_melaza ORDER BY pc_melaza_etapa ASC";
                                  $stmt_etapas = $conn->prepare($sql_etapas);
                                  $stmt_etapas->execute();
                                  $etapas = $stmt_etapas->fetchAll(PDO::FETCH_ASSOC);
                                  foreach ($etapas as $etapa_row) {
                                      echo '<option value="' . htmlspecialchars($etapa_row['pc_melaza_etapa']) . '">' . htmlspecialchars($etapa_row['pc_melaza_etapa']) . '</option>';
                                  }
                                  ?>
                              </select>
                          </span>                            
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="melazaTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $melazaQuery = "SELECT * FROM pc_melaza";
                      ...
                      if (empty($melazasData)) {
                          ...
                          foreach ($melazasData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-melaza" 
                                              ...
                                              data-melaza="' . htmlspecialchars($row['pc_melaza_nombre'] ?? '') . '" 
                                              data-etapa="' . htmlspecialchars($row['pc_melaza_etapa'] ?? '') . '" 
                                              data-racion="' . htmlspecialchars($row['pc_melaza_racion'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['pc_melaza_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['pc_melaza_vigencia'] ?? '') . '"
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['pc_melaza_nombre'] ?? '') . "</td>";
                              // Columna 2: Etapa
                              echo "<td>" . htmlspecialchars($row['pc_melaza_etapa'] ?? '') . "</td>";
                              
                              // Column 3: Dosis
                              echo "<td>" . htmlspecialchars($row['pc_melaza_racion'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Costo
                              echo "<td>" . htmlspecialchars($row['pc_melaza_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 5: Vigencia
                              echo "<td>" . htmlspecialchars($row['pc_melaza_vigencia'] ?? 'N/A') . "</td>";
