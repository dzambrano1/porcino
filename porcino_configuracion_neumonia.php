...
<title>Porcino Configuracion Neumonia</title>
<!-- Link to the Favicon -->
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando Neumonia</span>
...
<!-- New Entry Modal Configuracion Neumonia -->

<!-- Add New Vacuna Neumonia Button -->
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nueva Vacuna Neumonia
              </h5>
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="neumoniaTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $neumoniaQuery = "SELECT * FROM pc_neumonia";

                      $stmt = $conn->prepare($neumoniaQuery);
                      $stmt->execute();
                      $neumoniasData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($neumoniasData)) {
                          echo "<tr><td colspan='5' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          foreach ($neumoniasData as $row) {
                              echo "<tr>";
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-neumonia" 
                                              ...
                                              data-vacuna="' . htmlspecialchars($row['pc_neumonia_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['pc_neumonia_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['pc_neumonia_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['pc_neumonia_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Neumonia">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-neumonia" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Neumonia">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['pc_neumonia_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['pc_neumonia_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['pc_neumonia_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['pc_neumonia_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewNeumonia').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_neumonia.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-neumonia').click(function() {
        ...
        // Edit Configuracion Neumonia Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editNeumoniaModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Neumonia
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditNeumonia').click(function() {
            ...
            var form = document.getElementById('editNeumoniaForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de neumonia?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-neumonia').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de neumonia? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
