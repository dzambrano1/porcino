...
<title>Porcino Configuracion Peste</title>
<!-- Link to the Favicon -->
        ...
        <div class="col-12 col-md-6 col-lg-4">
            <div class="d-flex justify-content-center">
                <div class="arrow-step arrow-step-active w-100" style="border-radius: 15px; clip-path: none;">
                    <span class="badge-active">🎯 Estás configurando Peste</span>
      ...
      <div class="modal-content">
          <div class="modal-header">
              <h5 class="modal-title" id="newEntryModalLabel">
                  <i class="fas fa-plus-circle me-2"></i>Configurar Nuevo tratamiento Peste
              </h5>
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="pesteTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $pesteQuery = "SELECT * FROM pc_peste";
                      ...
                      if (empty($pestesData)) {
                          ...
                          foreach ($pestesData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-peste" 
                                              ...
                                              data-vacuna="' . htmlspecialchars($row['pc_peste_vacuna'] ?? '') . '" 
                                              data-dosis="' . htmlspecialchars($row['pc_peste_dosis'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['pc_peste_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['pc_peste_vigencia'] ?? '') . '"
                                              title="Editar Configuracion Vacuna Peste">
                                              <i class="fas fa-edit"></i>
                                          </button>';
                              echo '        <button class="btn btn-danger btn-sm delete-peste" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              title="Eliminar Configuracion Vacuna Peste">
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['pc_peste_vacuna'] ?? '') . "</td>";
                              
                              // Column 2: Dosis
                              echo "<td>" . htmlspecialchars($row['pc_peste_dosis'] ?? 'N/A') . "</td>";
                              
                              // Column 3: Costo
                              echo "<td>" . htmlspecialchars($row['pc_peste_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Vigencia
                              echo "<td>" . htmlspecialchars($row['pc_peste_vigencia'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    $('#saveNewPeste').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de peste ${formData.dosis} ml ?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: 'process_configuracion_peste.php',
                    type: 'POST',
                    data: {
                        action: 'insert',
    ...
    $('.edit-peste').click(function() {
        ...
        // Edit Configuracion Peste Modal dialog for editing
        var modalHtml = `
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editPesteModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacuna Peste
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditPeste').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la configuracion de peste?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-peste').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Está seguro de que desea eliminar la configuracion de peste? Esta acción no se puede deshacer.`,
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#dc3545',
