...
<title>Porcino Configuracion Sal</title>
<!-- Link to the Favicon -->
                      ...
                      <div class="input-group">
                          <span class="input-group-text">
                              ...
                              <select class="form-select" id="new_sal" name="sal" required>
                                  ...
                                  <?php
                                  $sql_names = "SELECT DISTINCT pc_sal_nombre FROM pc_sal ORDER BY pc_sal_nombre ASC";
                                  $stmt_names = $conn->prepare($sql_names);
                                  $stmt_names->execute();
                                  $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                  foreach ($names as $name_row) {
                                      echo '<option value="' . htmlspecialchars($name_row['pc_sal_nombre']) . '">' . htmlspecialchars($name_row['pc_sal_nombre']) . '</option>';
                                  }
                                  ?>
                              </select>
                          </span>                            
                      ...
                      <div class="input-group">
                          <span class="input-group-text">
                              ...
                              <select class="form-select" id="new_etapa" name="etapa" required>
                                  <option value="">Seleccionar</option>
                                  <?php
                                  $sql_etapas = "SELECT DISTINCT pc_sal_etapa FROM pc_sal ORDER BY pc_sal_etapa ASC";
                                  $stmt_etapas = $conn->prepare($sql_etapas);
                                  $stmt_etapas->execute();
                                  $etapas = $stmt_etapas->fetchAll(PDO::FETCH_ASSOC);
                                  foreach ($etapas as $etapa_row) {
                                      echo '<option value="' . htmlspecialchars($etapa_row['pc_sal_etapa']) . '">' . htmlspecialchars($etapa_row['pc_sal_etapa']) . '</option>';
                                  }
                                  ?>
                              </select>
                          </span>                            
                      ...
                      if (empty($salsData)) {
                          ...
                          foreach ($salsData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-sal" 
                                              ...
                                              data-sal="' . htmlspecialchars($row['pc_sal_nombre'] ?? '') . '" 
                                              data-etapa="' . htmlspecialchars($row['pc_sal_etapa'] ?? '') . '" 
                                              data-racion="' . htmlspecialchars($row['pc_sal_racion'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['pc_sal_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['pc_sal_vigencia'] ?? '') . '"
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['pc_sal_nombre'] ?? '') . "</td>";
                              // Columna 2: Etapa
                              echo "<td>" . htmlspecialchars($row['pc_sal_etapa'] ?? '') . "</td>";
                              
                              // Column 3: Dosis
                              echo "<td>" . htmlspecialchars($row['pc_sal_racion'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Costo
                              echo "<td>" . htmlspecialchars($row['pc_sal_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 5: Vigencia
                              echo "<td>" . htmlspecialchars($row['pc_sal_vigencia'] ?? 'N/A') . "</td>";
