...
<div class="container nav-icons-container">
    ...
    <div class="icon-button-container">
        <button onclick="window.location.href='./porcino_feria.php'" class="icon-button">
...
<div class="container text-center">
  ...
  <!-- DataTable for ph_aborto records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ph_abortoTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all ph_aborto records with animal name
                      $abortoQuery = "SELECT c.*, v.nombre as animal_nombre
                                FROM ph_aborto c 
                                LEFT JOIN porcino v ON c.ph_aborto_tagid = v.tagid 
                                ORDER BY c.ph_aborto_fecha DESC";
                      ...
                      if (empty($abortoData)) {
                          ...
                          foreach ($abortoData as $row) {
                              ...
                              try {
                                  $abortoDate = new DateTime($row['ph_aborto_fecha']);
                                  ...
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_aborto_fecha']);
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-aborto" 
                                          ...
                                          data-tagid="' . htmlspecialchars($row['ph_aborto_tagid'] ?? '') . '"
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.ph_aborto_tagid && !uniqueTagIds.has(item.ph_aborto_tagid)) {
                uniqueTagIds.add(item.ph_aborto_tagid);
                animals.push({
                    tagid: item.ph_aborto_tagid,
    ...
    function updateChart() {
        ...
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.ph_aborto_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.ph_aborto_fecha) - new Date(b.ph_aborto_fecha));
    ...
    function createAbortoChart(data) {
        ...
        var labels = data.map(function(item) {
            ...
            var parts = item.ph_aborto_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.ph_aborto_fecha;
        });
        
        var causa = data.map(function(item) {
            return item.ph_aborto_causa;
        });
