...
<title>Porcino - Registro Aftosa</title>
<!-- Link to the Favicon -->
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_aftosa_vacuna FROM pc_aftosa ORDER BY pc_aftosa_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_aftosa_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_aftosa_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $aftosaQuery = "SELECT 
                                          ...
                                          va.ph_aftosa_producto,
                                          va.ph_aftosa_dosis,
                                          va.ph_aftosa_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_aftosa_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_aftosa_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all aftosa records per tagid
                                          ph_aftosa va ON v.tagid = va.ph_aftosa_tagid 
                                      ORDER BY 
                                          va.ph_aftosa_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($aftosasData)) {
                          ...
                          try {
                              $configQuery = "SELECT p_vencimiento_aftosa FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_aftosa'])) {
                                  $vigencia = intval($row_config['p_vencimiento_aftosa']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($aftosasData as $row) {
                              ...
                              if ($row['in_aftosa_history'] == 1) {
                                  ...
                                  echo "<td>" . htmlspecialchars($row['ph_aftosa_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_aftosa_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_aftosa_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_aftosa_fecha'])) {
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['pc_aftosa_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_aftosa_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
