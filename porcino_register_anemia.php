...
<title>Porcino - Registro de Hierro</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Hierro</span>
...
<div class="container text-center">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nuevo Registro Hierro
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newAnemiaForm">
                <div class="mb-4">
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Hierro</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_anemia_vacuna FROM pc_anemia ORDER BY pc_anemia_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_anemia_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_anemia_vacuna']) . '</option>';
                                        }
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewAnemia">
  ...
  <!-- DataTable for Hierro records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="anemiaTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  ...
                  try {
                      $anemiaQuery = "SELECT 
                                          ...
                                          v.nombre,
                                          va.ph_anemia_fecha,
                                          va.id AS anemia_record_id, -- Alias anemia record ID
                                          va.ph_anemia_producto,
                                          va.ph_anemia_dosis,
                                          va.ph_anemia_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_anemia_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_anemia_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all anemia records per tagid
                                          ph_anemia va ON v.tagid = va.ph_anemia_tagid 
                                      ORDER BY 
                                          va.ph_anemia_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($anemiaQuery);
                      $stmt->execute();
                      $anemiasData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($anemiasData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for anemia records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT p_vencimiento_anemia FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_anemia'])) {
                                  $vigencia = intval($row_config['p_vencimiento_anemia']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($anemiasData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-anemia-btn" 
                              ...
                              if ($row['in_anemia_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-anemia" 
                                                  data-id="' . htmlspecialchars($row['anemia_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['ph_anemia_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ph_anemia_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ph_anemia_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ph_anemia_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                                  echo '        <button class="btn btn-danger btn-sm delete-anemia" 
                                                  data-id="' . htmlspecialchars($row['anemia_record_id'] ?? '') . '"
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_anemia_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_anemia_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_anemia_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_anemia_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_anemia_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_anemia_fecha'])) {
                                          $anemiaDate = new DateTime($row['ph_anemia_fecha']);
                                          $dueDate = clone $anemiaDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_anemia_fecha']);
                      ...
                      error_log("Error in anemia table: " . $e->getMessage());
...
<!-- Initialize DataTable for VH anemia -->
<script>
$(document).ready(function() {
    $('#anemiaTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editAnemiaModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewAnemia').click(function() {
        // Validate the form
        var form = document.getElementById('newAnemiaForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de anemia ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de anemia ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-anemia').click(function() {
        ...
        // Edit Anemia Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editAnemiaModal" tabindex="-1" aria-labelledby="editAnemiaModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editAnemiaModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Anemia
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editAnemiaForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Anemia</label>
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditAnemia">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Anemia
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['pc_anemia_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_anemia_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        // Show the edit modal
        var editModal = new bootstrap.Modal(document.getElementById('editAnemiaModal'));
        ...
        $('#saveEditAnemia').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de anemia para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $(document).on('click', '.register-new-anemia-btn', function() { 
    ...
    <div class="card shadow-lg border-0">
        ...
        <div class="card-body p-4">
            ...
            <div class="chart-container">
                ...
                <div id="chartError" class="text-center text-muted" style="display: none; padding: 2rem;">
                    <i class="fas fa-exclamation-triangle fa-3x mb-3"></i>
                    <h5>Error al cargar el gráfico</h5>
                    <p>No se pudo cargar el gráfico de evolución de dosis de hierro.</p>
...
<!-- Professional CSS Styling for Hierro Charts -->
<style>
...
<!-- Chart.js Script for Hierro Line Chart -->
<script>
$(document).ready(function() {
    let allAnemiaData = [];
    let anemiaChart = null;
    
    // Add loading state to chart container
    $('.chart-container').addClass('loading');
    
    // Debug chart container
    console.log('Chart container found:', $('.chart-container').length);
    console.log('Chart container dimensions:', {
        width: $('.chart-container').width(),
        height: $('.chart-container').height(),
        visible: $('.chart-container').is(':visible')
    });
    
    // Fetch anemia data and create the chart
    $.ajax({
        url: 'get_anemia_data.php',
        type: 'GET',
        dataType: 'json',
        beforeSend: function() {
            $('.chart-container').addClass('loading');
            console.log('Fetching anemia data...');
        },
        success: function(data) {
            $('.chart-container').removeClass('loading');
            console.log('Anemia data received:', data);
            ...
            if (!data || data.length === 0) {
                console.log('No anemia data available');
                return;
            }
            
            allAnemiaData = data;
            populateAnimalFilter(data);
            createAnemiaChart(data);
    ...
    function updateChart() {
        ...
        let filteredData = [...allAnemiaData];
    ...
    function createAnemiaChart(data) {
        console.log('Creating anemia chart with data:', data);
        
        var canvas = document.getElementById('anemiaChart');
        if (!canvas) {
            console.error('Canvas element "anemiaChart" not found!');
            return;
        }
        ...
        console.log('Chart dosis data:', dosis);
        
        console.log('Initializing Chart.js with data...');
        
        try {
            anemiaChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [{
        ...
        console.log('Anemia chart created successfully:', anemiaChart);
        } catch (error) {
            console.error('Error creating anemia chart:', error);
            $('#chartError').show();
            $('#anemiaChart').hide();
        }
    }
...
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4 fw-bold">
        ...
        Estado de Vacunación Anemia por Grupos
    </h4>
...
<!-- Chart.js Script for Anemia Status Pie Charts -->
<script>
$(document).ready(function() {
    const anemiaStatusUrl = 'get_anemia_status_by_grupo.php';
                ...
                <div class="card shadow-lg border-0 pie-chart-card">
                    ...
                    <div class="card-body p-4">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="anemiaPie${safeGrupo}"></canvas>
                            <div id="anemiaPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.error) {
                console.error('Error fetching anemia status data:', data.error);
            ...
            grupos.forEach((grupo, index) => {
                ...
                const canvasId = `anemiaPie${safeGrupo}`;
                const messageId = `anemiaPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching anemia status data:', error);
