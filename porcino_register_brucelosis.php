...
<title>Porcino - Registro de Vacunación Brucelosis</title>
<!-- Link to the Favicon -->
...
<style>
#editBrucelosisModal #editBrucelosisForm > .mb-2 {
    display: flex;
    align-items: center;
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_brucelosis_vacuna FROM pc_brucelosis ORDER BY pc_brucelosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_brucelosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_brucelosis_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $brucelosisQuery = "SELECT 
                                          ...
                                          p.tagid,
                                          p.nombre,
                                          pc.ph_brucelosis_fecha,
                                          pc.id AS brucelosis_record_id, -- Alias brucelosis record ID
                                          pc.ph_brucelosis_producto,
                                          pc.ph_brucelosis_dosis,
                                          pc.ph_brucelosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN pc.ph_brucelosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_brucelosis_history
                                      FROM 
                                          porcino p
                                      LEFT JOIN 
                                          -- Join with all brucelosis records per tagid
                                          ph_brucelosis pc ON p.tagid = pc.ph_brucelosis_tagid 
                                      ORDER BY 
                                          pc.ph_brucelosis_fecha DESC, p.tagid ASC;";
                      ...
                      if (empty($brucelosissData)) {
                          ...
                          try {
                              $configQuery = "SELECT p_vencimiento_brucelosis FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_brucelosis'])) {
                                  $vigencia = intval($row_config['p_vencimiento_brucelosis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($brucelosissData as $row) {
                              ...
                              if ($row['in_brucelosis_history'] == 1) {
                                  ...
                                  echo "<td>" . htmlspecialchars($row['ph_brucelosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_brucelosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_brucelosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_brucelosis_fecha'])) {
...
$(document).ready(function() {
    ...
    $('.edit-brucelosis').click(function() {
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editBrucelosisModalLabel">
                            <i class="fas fa-syringe me-2"></i>Editar Dosis de brucelosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
        ...
        $('#saveEditBrucelosis').click(function() {
            // Validate the form
            var form = document.getElementById('editBrucelosisForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la dosis de brucelosis ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            Swal.fire({
                                ...
                                text: 'El registro de brucelosis ha sido actualizado correctamente',
                                icon: 'success',
                                confirmButtonColor: '#28a745'
                            }).then(() => {
                                // Reload the page to show updated data
