...
<title>Porcino - Registro Carbunco</title>
<!-- Link to the Favicon -->
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_carbunco_vacuna FROM pc_carbunco ORDER BY pc_carbunco_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_carbunco_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_carbunco_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $carbuncoQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.ph_carbunco_fecha,
                                          va.id AS carbunco_record_id, -- Alias carbunco record ID
                                          va.ph_carbunco_producto,
                                          va.ph_carbunco_dosis,
                                          va.ph_carbunco_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_carbunco_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_carbunco_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all carbunco records per tagid
                                          ph_carbunco va ON v.tagid = va.ph_carbunco_tagid 
                                      ORDER BY 
                                          va.ph_carbunco_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($carbuncoData)) {
                          ...
                          try {
                              $configQuery = "SELECT p_vencimiento_carbunco FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_carbunco'])) {
                                  $vigencia = intval($row_config['p_vencimiento_carbunco']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($carbuncoData as $row) {
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_carbunco_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_carbunco_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_carbunco_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_carbunco_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_carbunco_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_carbunco_fecha'])) {
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['pc_carbunco_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_carbunco_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
