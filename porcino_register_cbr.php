...
<title>Porcino - Registro de Vacunación CBR</title>
<!-- Link to the Favicon -->
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_cbr_vacuna FROM pc_cbr ORDER BY pc_cbr_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_cbr_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_cbr_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $cbrQuery = "SELECT 
                                          ...
                                          CASE WHEN va.ph_cbr_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_cbr_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all cbr records per tagid
                                          ph_cbr va ON v.tagid = va.ph_cbr_tagid 
                                      ORDER BY 
                                          va.ph_cbr_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($cbrsData)) {
                          ...
                          try {
                              $configQuery = "SELECT p_vencimiento_cbr FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_cbr'])) {
                                  $vigencia = intval($row_config['p_vencimiento_cbr']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($cbrsData as $row) {
                              ...
                              if ($row['in_cbr_history'] == 1) {
                                  ...
                                  echo "<td>" . htmlspecialchars($row['ph_cbr_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_cbr_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_cbr_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_cbr_fecha'])) {
