...
<title>Porcino - Registro de Coccidiosis</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Coccidiosis
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newCoccidiosisForm">
                <div class="mb-4">
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Coccidiosis</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_coccidiosis_vacuna FROM pc_coccidiosis ORDER BY pc_coccidiosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_coccidiosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_coccidiosis_vacuna']) . '</option>';
                                        }
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewCoccidiosis">
  ...
  <!-- DataTable for ph_coccidiosis records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="coccidiosisTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      $coccidiosisQuery = "SELECT 
                                          v.id AS porcino_id, -- Alias porcino ID
                                          v.tagid,
                                          v.nombre,
                                          va.ph_coccidiosis_fecha,
                                          va.id AS coccidiosis_record_id, -- Alias coccidiosis record ID
                                          va.ph_coccidiosis_producto,
                                          va.ph_coccidiosis_dosis,
                                          va.ph_coccidiosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_coccidiosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_coccidiosis_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all coccidiosis records per tagid
                                          ph_coccidiosis va ON v.tagid = va.ph_coccidiosis_tagid 
                                      ORDER BY 
                                          va.ph_coccidiosis_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($coccidiosisQuery);
                      $stmt->execute();
                      $coccidiosisData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($coccidiosisData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for coccidiosis records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT p_vencimiento_coccidiosis FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_coccidiosis'])) {
                                  $vigencia = intval($row_config['p_vencimiento_coccidiosis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($coccidiosisData as $row) {
                              echo "<tr>";
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-coccidiosis-btn" 
                              ...
                              if ($row['in_coccidiosis_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-coccidiosis" 
                                                  data-id="' . htmlspecialchars($row['coccidiosis_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['ph_coccidiosis_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ph_coccidiosis_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ph_coccidiosis_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ph_coccidiosis_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                                  echo '        <button class="btn btn-danger btn-sm delete-coccidiosis" 
                                                  data-id="' . htmlspecialchars($row['coccidiosis_record_id'] ?? '') . '"
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_coccidiosis_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_coccidiosis_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_coccidiosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_coccidiosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_coccidiosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_coccidiosis_fecha'])) {
                                          $coccidiosisDate = new DateTime($row['ph_coccidiosis_fecha']);
                                          $dueDate = clone $coccidiosisDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_coccidiosis_fecha']);
                      ...
                      error_log("Error in coccidiosis table: " . $e->getMessage());
...
<!-- Initialize DataTable for VH coccidiosis -->
<script>
$(document).ready(function() {
    $('#coccidiosisTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editcoccidiosisModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewCoccidiosis').click(function() {
        // Validate the form
        var form = document.getElementById('newCoccidiosisForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de coccidiosis ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de coccidiosis ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-coccidiosis').click(function() {
        var id = $(this).data('id');
        ...
        // Edit coccidiosis Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editCoccidiosisModal" tabindex="-1" aria-labelledby="editCoccidiosisModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editCoccidiosisModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Coccidiosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editCoccidiosisForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Coccidiosis</label>
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditCoccidiosis">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Coccidiosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['pc_coccidiosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_coccidiosis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        // Show the edit modal
        var editModal = new bootstrap.Modal(document.getElementById('editCoccidiosisModal'));
        ...
        $('#saveEditCoccidiosis').click(function() {
            var formData = {
                id: $('#edit_id').val(),
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de coccidiosis para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-coccidiosis').click(function() {
        var id = $(this).data('id');
        var tagid = $(this).data('tagid');
    ...
    $(document).on('click', '.register-new-coccidiosis-btn', function() { 
...
<!-- coccidiosis Line Chart Section -->
    ...
    <div class="card shadow-lg border-0">
        ...
        <div class="card-body p-4">
            ...
            <div class="chart-container">
                <canvas id="coccidiosisChart"></canvas>
            </div>
        </div>
    </div>
</div>

<!-- Professional CSS Styling for Coccidiosis Charts -->
<style>
...
<!-- Chart.js Script for coccidiosis Line Chart -->
<script>
$(document).ready(function() {
    let allCoccidiosisData = [];
    let coccidiosisChart = null;
    
    // Add loading state to chart container
    $('.chart-container').addClass('loading');
    
    // Fetch coccidiosis data and create the chart
    $.ajax({
        url: 'get_coccidiosis_data.php',
        type: 'GET',
        dataType: 'json',
        beforeSend: function() {
            $('.chart-container').addClass('loading');
        ...
        success: function(data) {
            ...
            allCoccidiosisData = data;
            populateAnimalFilter(data);
            createCoccidiosisChart(data);
        ...
        error: function(xhr, status, error) {
            $('.chart-container').removeClass('loading');
            console.error('Error fetching coccidiosis data:', error);
        }
    });
    ...
    function updateChart() {
        ...
        let filteredData = [...allCoccidiosisData];
    ...
    function updateChartData(data) {
        if (coccidiosisChart) {
            coccidiosisChart.destroy();
        }
        createCoccidiosisChart(data);
    }
    
    function createCoccidiosisChart(data) {
        var ctx = document.getElementById('coccidiosisChart').getContext('2d');
        ...
        coccidiosisChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [{
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Coccidiosis por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for coccidiosis Status Pie Charts -->
<script>
$(document).ready(function() {
    const coccidiosisStatusUrl = 'get_coccidiosis_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="coccidiosisPie${safeGrupo}"></canvas>
                            <div id="coccidiosisPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        url: coccidiosisStatusUrl,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.error) {
                console.error('Error fetching coccidiosis status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `coccidiosisPie${safeGrupo}`;
                const messageId = `coccidiosisPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching coccidiosis status data:', error);
