...
try {
    // Fetch all purchases ordered by date from porcino table
...
<title>Porcino Register Compras</title>
<!-- Link to the Favicon -->
...
.btn-professional-register:focus {
    outline: none;
    box-shadow: 0 0 0 3px rgba(40, 167, 69, 0.3), 0 10px 30px rgba(40, 167, 69, 0.4);
}

.btn-professional-register:focus:not(:focus-visible) {
    box-shadow: 0 10px 30px rgba(40, 167, 69, 0.4);
}
</style>
</head>
                    ...
                    <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="etapa" id="newEtapa" required>
                                            ...
                                            $sql_etapa = "SELECT DISTINCT pc_etapas_nombre FROM pc_etapas ORDER BY pc_etapas_nombre";
                                            $result_etapa = $conn_etapa->query($sql_etapa);
                                            while ($row_etapa = $result_etapa->fetch_assoc()) {
                                                    echo '<option value="' . htmlspecialchars($row_etapa['pc_etapas_nombre']) . '">' 
                                                    . htmlspecialchars($row_etapa['pc_etapas_nombre']) . '</option>';
                                            }
                                            $conn_etapa->close();
                                            ?>
                                        </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-floating">
                                ...
                                <label for="newPrecio">Precio (Kg)</label>
                                <div class="invalid-feedback">
                                    Por favor ingrese un precio por Kg.
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="form-floating">
                                ...
                                <label for="updatePrecio">Precio (Kg)</label>
                            </div>
                        </div>
                    </div>
                </form>
      ...
      <div class="table-responsive">
          <table id="compraTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get purchase data directly from porcino table
                      $compraQuery = "SELECT 
                                        id, 
                  ...
                  foreach ($compraData as $row) {
                      ...
                      echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>"; // porcino.nombre                      
                      echo "<td class='text-center'>" . htmlspecialchars($row['fecha_compra_formatted'] ?? 'N/A') . "</td>"; // Use formatted fecha_compra
                      echo "<td class='text-center'>" . htmlspecialchars($row['tagid'] ?? '') . "</td>"; // porcino.tagid
                      echo "<td class='text-center'>" . htmlspecialchars($row['precio_compra'] ?? '0.00') . "</td>"; // porcino.precio_compra
                      echo "<td class='text-center'>" . htmlspecialchars($row['peso_compra'] ?? '0.00') . "</td>"; // porcino.peso_compra
                      echo "</tr>";
                  }
                  ?>
              </tbody>
...
$(document).ready(function() {
    ...
    $('.edit-compra').click(function() {
        ...
        $.ajax({
            url: 'porcino_get_details.php', // Assuming this endpoint exists
            type: 'GET',
            data: { tagid: tagid },
            dataType: 'json',
