...
<div class="container text-center">
  ...
  <!-- DataTable for ph_compra records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ph_compraTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                    // Query to get all ph_compra records with animal name
                    $compraQuery = "SELECT c.*, v.nombre as animal_nombre
                              FROM ph_compra c 
                              LEFT JOIN porcino v ON c.ph_compra_tagid = v.tagid 
                              ORDER BY c.ph_compra_fecha DESC";
                    ...
                    if (empty($compraData)) {
                        ...
                        foreach ($compraData as $row) {
                            ...
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_compra_fecha'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_compra_tagid'] ?? '') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_compra_precio'] ?? '') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_compra_peso'] ?? '') . "</td>";

                            // Calculate due date and determine status
                            try {
                                $compraDate = new DateTime($row['ph_compra_fecha']);
                                ...
                                error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_compra_fecha']);
                            ...
                            echo '<td class="text-center">
                                <div class="btn-group" role="group">
                                    <button class="btn btn-warning btn-sm edit-compra" 
                                        ...
                                        data-tagid="' . htmlspecialchars($row['ph_compra_tagid'] ?? '') . '"
                                        data-fecha="' . htmlspecialchars($row['ph_compra_fecha'] ?? '') . '"
                                        data-precio="' . htmlspecialchars($row['ph_compra_precio'] ?? '') . '"
                                        data-peso="' . htmlspecialchars($row['ph_compra_peso'] ?? '') . '">
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.ph_compra_tagid && !uniqueTagIds.has(item.ph_compra_tagid)) {
                uniqueTagIds.add(item.ph_compra_tagid);
                animals.push({
                    tagid: item.ph_compra_tagid,
    ...
    function updateChart() {
        ...
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.ph_compra_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.ph_compra_fecha) - new Date(b.ph_compra_fecha));
    ...
    function createCompraChart(data) {
        ...
        var labels = data.map(function(item) {
            ...
            var parts = item.ph_compra_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.ph_compra_fecha;
        });
        
        var precio = data.map(function(item) {
            return item.ph_compra_precio;
        });
...
<div class="container text-center">
  ...
  <!-- DataTable for ph_deceso records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ph_decesoTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th class="text-center">ID</th>
                      <th class="text-center">Fecha</th>
              ...
              <tbody>
                  <?php
                  try {
                    // Query to get all ph_parto records with animal name
                    $decesoQuery = "SELECT c.*, v.nombre as animal_nombre
                              FROM ph_deceso c 
                              LEFT JOIN porcino v ON c.ph_deceso_tagid = v.tagid 
                              ORDER BY c.ph_deceso_fecha DESC";
                    ...
                    if (empty($decesoData)) {
                        ...
                        foreach ($decesoData as $row) {
                            echo "<tr>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['id'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_deceso_fecha'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_deceso_tagid'] ?? '') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_deceso_causa'] ?? '') . "</td>";
                            // Calculate due date and determine status
                            try {
                                $decesoDate = new DateTime($row['ph_deceso_fecha']);
                                ...
                                // Handle invalid date format
                                error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_deceso_fecha']);
                                echo '<td class="text-center"><span class="badge bg-warning">ERROR</span></td>';
                            }
                            ...
                            echo '<td class="text-center">
                                <div class="btn-group" role="group">
                                    <button class="btn btn-warning btn-sm edit-deceso" 
                                        data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                        data-tagid="' . htmlspecialchars($row['ph_deceso_tagid'] ?? '') . '"
                                        data-fecha="' . htmlspecialchars($row['ph_deceso_fecha'] ?? '') . '"
                                        data-peso="' . htmlspecialchars($row['ph_deceso_peso'] ?? '') . '">
                                        <i class="fas fa-edit"></i>
                                    </button>
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.ph_deceso_tagid && !uniqueTagIds.has(item.ph_deceso_tagid)) {
                uniqueTagIds.add(item.ph_deceso_tagid);
                animals.push({
                    tagid: item.ph_deceso_tagid,
                    nombre: item.animal_nombre || 'Sin nombre'
                });
            }
        });
    ...
    function updateChart() {
        ...
        // Filter by animal if not "all"
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.ph_deceso_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.ph_deceso_fecha) - new Date(b.ph_deceso_fecha));
    ...
    function createDecesoChart(data) {
        ...
        var labels = data.map(function(item) {
            // Format the date for display
            var parts = item.ph_deceso_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.ph_deceso_fecha;
        });
        
        var causa = data.map(function(item) {
            return item.ph_deceso_causa;
        });
...
<div class="container text-center">
  ...
  <!-- DataTable for ph_descarte records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ph_descarteTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th class="text-center">ID</th>
                      <th class="text-center">Fecha</th>
              ...
              <tbody>
                  ...
                  try {
                    ...
                    $descarteQuery = "SELECT c.*, v.nombre as animal_nombre
                              FROM ph_descarte c 
                              LEFT JOIN porcino v ON c.ph_descarte_tagid = v.tagid 
                              ORDER BY c.ph_descarte_fecha DESC";
                    ...
                    if (empty($descarteData)) {
                        ...
                        foreach ($descarteData as $row) {
                            echo "<tr>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['id'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_descarte_fecha'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_descarte_tagid'] ?? '') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['ph_descarte_peso'] ?? '') . "</td>";
                            
                            // Calculate due date and determine status
                            try {
                                $descarteDate = new DateTime($row['ph_descarte_fecha']);
                                ...
                                // Handle invalid date format
                                error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_descarte_fecha']);
                                echo '<td class="text-center"><span class="badge bg-warning">ERROR</span></td>';
                            }
                            ...
                            echo '<td class="text-center">
                                <div class="btn-group" role="group">
                                    <button class="btn btn-warning btn-sm edit-descarte" 
                                        data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                        data-tagid="' . htmlspecialchars($row['ph_descarte_tagid'] ?? '') . '"
                                        data-fecha="' . htmlspecialchars($row['ph_descarte_fecha'] ?? '') . '"
                                        data-peso="' . htmlspecialchars($row['ph_descarte_peso'] ?? '') . '">
                                        <i class="fas fa-edit"></i>
                                    </button>
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.ph_descarte_tagid && !uniqueTagIds.has(item.ph_descarte_tagid)) {
                uniqueTagIds.add(item.ph_descarte_tagid);
                animals.push({
                    tagid: item.ph_descarte_tagid,
                    nombre: item.animal_nombre || 'Sin nombre'
                });
            }
        });
    ...
    function updateChart() {
        ...
        // Filter by animal if not "all"
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.ph_descarte_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.ph_descarte_fecha) - new Date(b.ph_descarte_fecha));
    ...
    function createDescarteChart(data) {
        ...
        var labels = data.map(function(item) {
            // Format the date for display
            var parts = item.ph_descarte_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.ph_descarte_fecha;
        });
        
        var causa = data.map(function(item) {
            return item.ph_descarte_causa;
        });
...
<div class="container text-center">
  ...
  <!-- DataTable for ph_venta records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ph_ventaTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th class="text-center">ID</th>
                      <th class="text-center">Fecha</th>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all ph_venta records with animal name
                      $ventaQuery = "SELECT v.*, v.nombre as animal_nombre,
                                ve.ph_venta_peso as ph_venta_peso,
                                ve.ph_venta_precio as ph_venta_precio,
                                ve.ph_venta_fecha as ph_venta_fecha,
                                ve.ph_venta_tagid as ph_venta_tagid
                                FROM ph_venta ve 
                                JOIN porcino v ON v.tagid = ve.ph_venta_tagid 
                                ORDER BY ve.ph_venta_fecha DESC";
                      ...
                      if (empty($ventaData)) {
                          ...
                          foreach ($ventaData as $row) {
                              echo "<tr>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['id'] ?? '') . "</td>";                        
                              echo "<td class='text-center'>" . htmlspecialchars($row['ph_venta_fecha'] ?? '') . "</td>";                        
                              echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['ph_venta_tagid'] ?? '') . "</td>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['ph_venta_peso'] ?? '') . "</td>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['ph_venta_precio'] ?? '') . "</td>";

                              // Calculate due date and determine status
                              try {
                                  $ventaDate = new DateTime($row['ph_venta_fecha']);
                                  ...
                                  // Handle invalid date format
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_venta_fecha']);
                                  echo '<td class="text-center"><span class="badge bg-warning">ERROR</span></td>';
                              }
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-venta" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              data-tagid="' . htmlspecialchars($row['ph_venta_tagid'] ?? '') . '"
                                              data-fecha="' . htmlspecialchars($row['ph_venta_fecha'] ?? '') . '"
                                              data-peso="' . htmlspecialchars($row['ph_venta_peso'] ?? '') . '"
                                          data-precio="' . htmlspecialchars($row['ph_venta_precio'] ?? '') . '">
                                          <i class="fas fa-edit"></i>
                                      </button>
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.ph_venta_tagid && !uniqueTagIds.has(item.ph_venta_tagid)) {
                uniqueTagIds.add(item.ph_venta_tagid);
                animals.push({
                    tagid: item.ph_venta_tagid,
                    nombre: item.animal_nombre || 'Sin nombre'
                });
            }
        });
    ...
    function updateChart() {
        ...
        // Filter by animal if not "all"
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.ph_venta_tagid === selectedAnimal);
        }   
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.ph_venta_fecha) - new Date(b.ph_venta_fecha));
    ...
    function createVentaChart(data) {
        ...
        var labels = data.map(function(item) {
            // Format the date for display
            var parts = item.ph_venta_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.ph_venta_fecha;
        });
        
        var precio = data.map(function(item) {
            return item.ph_venta_precio;
        });
