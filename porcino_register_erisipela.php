...
<title>Porcino - Registro de Erisipela</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Erisipela</span>
              ...
              <tbody>
                  ...
                  try {
                      $erisipelaQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.ph_erisipela_fecha,
                                          va.id AS erisipela_record_id, -- Alias erisipela record ID
                                          va.ph_erisipela_producto,
                                          va.ph_erisipela_dosis,
                                          va.ph_erisipela_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_erisipela_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_erisipela_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all erisipela records per tagid
                                          ph_erisipela va ON v.tagid = va.ph_erisipela_tagid 
                                      ORDER BY 
                                          va.ph_erisipela_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($erisipelaQuery);
                      $stmt->execute();
                      $erisipelaData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($erisipelaData)) {
...
$(document).ready(function() {
    ...
    $('.edit-erisipela').click(function() {
        ...
        // Edit erisipela Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editErisipelaModal" tabindex="-1" aria-labelledby="editErisipelaModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editErisipelaModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Erisipela
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editErisipelaForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Erisipela</label>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Erisipela
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['pc_erisipela_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_erisipela_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        $('#saveEditErisipela').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de erisipela para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
...
<!-- Professional CSS Styling for Erisipela Charts -->
<style>
