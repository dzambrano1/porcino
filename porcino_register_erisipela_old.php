...
<title>Porcino - Registro de Vacunación Erisipela</title>
<!-- Link to the Favicon -->
...
<style>
#editErisipelaModal #editErisipelaForm > .mb-2 {
    display: flex;
    align-items: center;
...
<div class="container text-center">
  ...
  REGISTROS DE ERISIPELA
  </h3>
  
  <!-- New Erisipela Entry Modal -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Erisipela
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newErisipelaForm">
                <div class="mb-2">
                    ...
                    <div class="mb-2">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Erisipela</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_erisipela_vacuna FROM pc_erisipela ORDER BY pc_erisipela_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_erisipela_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_erisipela_vacuna']) . '</option>';
                                        }
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewErisipela">
  ...
  <!-- DataTable for ph_erisipela records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="erisipelaTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      $erisipelaQuery = "SELECT 
                                          p.id AS porcino_id, -- Alias porcino ID
                                          p.tagid,
                                          p.nombre,
                                          pc.ph_erisipela_fecha,
                                          pc.id AS erisipela_record_id, -- Alias erisipela record ID
                                          pc.ph_erisipela_producto,
                                          pc.ph_erisipela_dosis,
                                          pc.ph_erisipela_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN pc.ph_erisipela_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_erisipela_history
                                      FROM 
                                          porcino p
                                      LEFT JOIN 
                                          -- Join with all erisipela records per tagid
                                          ph_erisipela pc ON p.tagid = pc.ph_erisipela_tagid 
                                      ORDER BY 
                                          pc.ph_erisipela_fecha DESC, p.tagid ASC;";

                      $stmt = $conn->prepare($erisipelaQuery);
                      $stmt->execute();
                      $erisipelasData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($erisipelasData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for erisipela records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT p_vencimiento_erisipela FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_erisipela'])) {
                                  $vigencia = intval($row_config['p_vencimiento_erisipela']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($erisipelasData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-erisipela-btn" 
                              ...
                              if ($row['in_erisipela_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-erisipela" 
                                                  data-id="' . htmlspecialchars($row['erisipela_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['ph_erisipela_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ph_erisipela_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ph_erisipela_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ph_erisipela_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                                  echo '        <button class="btn btn-danger btn-sm delete-erisipela" 
                                                  data-id="' . htmlspecialchars($row['erisipela_record_id'] ?? '') . '"
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_erisipela_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_erisipela_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_erisipela_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_erisipela_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_erisipela_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_erisipela_fecha'])) {
                                          $erisipelaDate = new DateTime($row['ph_erisipela_fecha']);
                                          $dueDate = clone $erisipelaDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_erisipela_fecha']);
                      ...
                      error_log("Error in erisipela table: " . $e->getMessage());
...
<!-- Initialize DataTable for VH erisipela -->
<script>
$(document).ready(function() {
    $('#erisipelaTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editErisipelaModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewErisipela').click(function() {
        // Validate the form
        var form = document.getElementById('newErisipelaForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de erisipela ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de erisipela ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-erisipela').click(function() {
        ...
        // edit modal HTML
        var modalHtml = `
        <div class="modal fade" id="editErisipelaModal" tabindex="-1" aria-labelledby="editErisipelaModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editErisipelaModalLabel">
                            <i class="fas fa-syringe me-2"></i>Editar Dosis de erisipela
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editErisipelaForm">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditErisipela">
        ...
        $('#editErisipelaModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editErisipelaModal'));
        editModal.show();
        ...
        $('#saveEditErisipela').click(function() {
            // Validate the form
            var form = document.getElementById('editErisipelaForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la dosis de erisipela ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            Swal.fire({
                                ...
                                text: 'El registro de erisipela ha sido actualizado correctamente',
                                icon: 'success',
                                confirmButtonColor: '#28a745'
                            }).then(() => {
                                // Reload the page to show updated data
    ...
    $('.delete-erisipela').click(function() {
        var id = $(this).data('id');
        var tagid = $(this).data('tagid');
    ...
    $(document).on('click', '.register-new-erisipela-btn', function() { 
...
<!-- Erisipela Line Chart Section -->

<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-success text-white">
            <h5 class="mb-0"><i class="fas fa-chart-line me-2"></i>Evolución Dosis Erisipela</h5>
        </div>
        <div class="card-body">
            <div class="row mb-2">
                <div class="col-md-4">
            ...
            <div class="chart-container" style="position: relative; height:50vh; width:100%">
                <canvas id="erisipelaChart"></canvas>
            </div>
        </div>
    </div>
</div>

<!-- Chart.js Script for Erisipela Line Chart -->
<script>
$(document).ready(function() {
    let allErisipelaData = [];
    let erisipelaChart = null;
    
    // Fetch erisipela data and create the chart
    $.ajax({
        url: 'get_erisipela_data.php',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            ...
            allErisipelaData = data;
            populateAnimalFilter(data);
            createErisipelaChart(data);
        ...
        error: function(xhr, status, error) {
            console.error('Error fetching erisipela data:', error);
        }
    });
    ...
    function updateChart() {
        ...
        let filteredData = [...allErisipelaData];
    ...
    function updateChartData(data) {
        if (erisipelaChart) {
            erisipelaChart.destroy();
        }
        createErisipelaChart(data);
    }
    
    function createErisipelaChart(data) {
        var ctx = document.getElementById('erisipelaChart').getContext('2d');
                    ...
                    title: {
                        ...
                        text: function() {
                            ...
                            if (selectedAnimal !== 'all') {
                                ...
                                return 'Evolución de Dosis de Erisipela - ' + animalName;
                            }
                            return 'Evolución de Dosis de Erisipela - Todos los Animales';
                        },
                        font: {
                            size: 16
                        }
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Erisipela por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for Erisipela Status Pie Charts -->
<script>
$(document).ready(function() {
    const erisipelaStatusUrl = 'get_erisipela_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="erisipelaPie${safeGrupo}"></canvas>
                            <div id="erisipelaPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        url: erisipelaStatusUrl,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.error) {
                console.error('Error fetching erisipela status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `erisipelaPie${safeGrupo}`;
                const messageId = `erisipelaPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching erisipela status data:', error);
