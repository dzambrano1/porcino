...
try {
    ...
    $concentradoQuery = "SELECT 
                            ph_concentrado_fecha_inicio,
                            ph_concentrado_fecha_fin,
                            ph_concentrado_racion,
                            ph_concentrado_costo,
                            ph_concentrado_tagid
                        FROM ph_concentrado 
                        WHERE ph_concentrado_fecha_inicio IS NOT NULL 
                        AND ph_concentrado_fecha_fin IS NOT NULL 
                        AND ph_concentrado_racion IS NOT NULL 
                        AND ph_concentrado_costo IS NOT NULL
                        ORDER BY ph_concentrado_fecha_inicio ASC";
    ...
    if (empty($concentradoData)) {
        ...
        $concentradoQuery = "SELECT 
                                ph_concentrado_fecha,
                                ph_concentrado_fecha as ph_concentrado_fecha_inicio,
                                ph_concentrado_fecha as ph_concentrado_fecha_fin,
                                ph_concentrado_racion,
                                ph_concentrado_costo,
                                ph_concentrado_tagid
                            FROM ph_concentrado 
                            WHERE ph_concentrado_fecha IS NOT NULL 
                            AND ph_concentrado_racion IS NOT NULL 
                            AND ph_concentrado_costo IS NOT NULL
                            ORDER BY ph_concentrado_fecha ASC";
    ...
    foreach ($concentradoData as $concentrado) {
        $startDate = new DateTime($concentrado['ph_concentrado_fecha_inicio']);
        $endDate = new DateTime($concentrado['ph_concentrado_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $concentrado['ph_concentrado_racion'] * $concentrado['ph_concentrado_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing concentrado: Start=" . $concentrado['ph_concentrado_fecha_inicio'] . 
                 ", End=" . $concentrado['ph_concentrado_fecha_fin'] . 
                 ", Days=" . $totalDays . 
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Animals and ALL their concentrado records (if any)
                        $concentradoQuery = "
                            SELECT
                                v.tagid AS porcino_tagid,
                                v.nombre AS animal_nombre,
                                v.etapa AS porcino_etapa,
                            ...
                            FROM
                                porcino v
                            LEFT JOIN
                                ph_concentrado c ON v.tagid = c.ph_concentrado_tagid -- Join ALL matching concentrado records
                            ORDER BY
                      ...
                      if (empty($concentradoData)) {
                          ...
                          foreach ($concentradoData as $row) {
                              ...
                              $concentradoFechaInicio = $row['ph_concentrado_fecha_inicio'] ?? null;
                              $concentradoFechaFin = $row['ph_concentrado_fecha_fin'] ?? null;
                              ...
                              if ($hasConcentrado) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-concentrado" 
                                                  ...
                                                  data-etapa="'.htmlspecialchars($row['porcino_etapa'] ?? '').'" 
                                                  ...
                                                  data-debug="DB: '.htmlspecialchars($row['ph_concentrado_producto'] ?? 'NULL').'">
                              ...
                              echo "<td>" . htmlspecialchars($row['porcino_etapa'] ?? 'N/A') . "</td>";
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['pc_concentrado_nombre']) . '">' . htmlspecialchars($alimento_row['pc_concentrado_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching alimentos: " . $e->getMessage());
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="etapaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $etapas = $stmt_etapas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($etapas as $etapa_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action etapa-option" data-value="' . htmlspecialchars($etapa_row['pc_etapas_nombre']) . '">' . htmlspecialchars($etapa_row['pc_etapas_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching etapas: " . $e->getMessage());
