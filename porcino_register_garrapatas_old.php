              ...
              <tbody>
                  ...
                  try {
                      $garrapatasQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.ph_garrapatas_fecha,
                                          va.id AS garrapatas_record_id, -- Alias garrapatas record ID
                                          va.ph_garrapatas_producto,
                                          va.ph_garrapatas_dosis,
                                          va.ph_garrapatas_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_garrapatas_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_garrapatas_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all garrapatas records per tagid
                                          ph_garrapatas va ON v.tagid = va.ph_garrapatas_tagid 
                                      ORDER BY 
                                          va.ph_garrapatas_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($garrapatassData)) {
                          ...
                          foreach ($garrapatassData as $row) {
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_garrapatas_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_garrapatas_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_garrapatas_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_garrapatas_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_garrapatas_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_garrapatas_fecha'])) {
        ...
        garrapatasChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Evolución de Dosis de Garrapatas',
                        font: {
                            size: 16
                        }
                    }
            ...
            console.error('Error creating garrapatas chart:', error);
            $('#garrapatasChart').parent().html('<div class="alert alert-danger">Error al crear el gráfico: ' + error.message + '</div>');
        }
    }
});
</script>
