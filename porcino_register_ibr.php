              ...
              <tbody>
                  ...
                  try {
                      $ibrQuery = "SELECT 
                                          ...
                                          CASE WHEN va.ph_ibr_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_ibr_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all ibr records per tagid
                                          ph_ibr va ON v.tagid = va.ph_ibr_tagid 
                                      ORDER BY 
                                          va.ph_ibr_fecha DESC, v.tagid ASC;";
                          ...
                          foreach ($ibrsData as $row) {
                              ...
                              if ($row['in_ibr_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-ibr" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['ph_ibr_producto'] ?? '') . '" 
                              ...
                              if ($row['in_ibr_history'] == 1) {
                                  ...
                                  echo "<td>" . htmlspecialchars($row['ph_ibr_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_ibr_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_ibr_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_ibr_fecha'])) {
