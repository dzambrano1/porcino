...
<title>Porcino - Registro Leptopirosis</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Leptopirosis</span>
              ...
              <tbody>
                  ...
                  try {
                      $leptopirosisQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.ph_leptopirosis_fecha,
                                          va.id AS leptopirosis_record_id, -- Alias leptopirosis record ID
                                          va.ph_leptopirosis_producto,
                                          va.ph_leptopirosis_dosis,
                                          va.ph_leptopirosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_leptopirosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_leptopirosis_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all leptopirosis records per tagid
                                          ph_leptopirosis va ON v.tagid = va.ph_leptopirosis_tagid 
                                      ORDER BY 
                                          va.ph_leptopirosis_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($leptopirosisQuery);
                      $stmt->execute();
                      $leptopirosisData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($leptopirosisData)) {
                          ...
                          foreach ($leptopirosisData as $row) {
                              echo "<tr>";
...
$(document).ready(function() {
    ...
    $('.edit-leptopirosis').click(function() {
        ...
        // Edit leptopirosis Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editLeptopirosisModal" tabindex="-1" aria-labelledby="editLeptopirosisModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editLeptopirosisModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Leptopirosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editLeptopirosisForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Leptopirosis</label>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Leptopirosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['pc_leptopirosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_leptopirosis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        $('#saveEditLeptopirosis').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de leptopirosis para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
...
<!-- Professional CSS Styling for Leptopirosis Charts -->
<style>
