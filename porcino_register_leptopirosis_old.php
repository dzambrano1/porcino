...
<title>Porcino - Registro de Vacunación Leptopirosis</title>
<!-- Link to the Favicon -->
...
<style>
#editLeptopirosisModal #editLeptopirosisForm > .mb-2 {
    display: flex;
    align-items: center;
...
<div class="container text-center">
  <h3  class="container mt-4 text-white" class="collapse" id="section-historial-produccion-porcino">
  REGISTROS DE LEPTOPIROSIS
  </h3>
  
  <!-- New Leptopirosis Entry Modal -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Leptopirosis
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newLeptopirosisForm">
                <div class="mb-2">
                    ...
                    <div class="mb-2">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Leptopirosis</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_leptopirosis_vacuna FROM pc_leptopirosis ORDER BY pc_leptopirosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_leptopirosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_leptopirosis_vacuna']) . '</option>';
                                        }
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewLeptopirosis">
  ...
  <!-- DataTable for ph_leptopirosis records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="leptopirosisTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      $leptopirosisQuery = "SELECT 
                                          p.id AS porcino_id, -- Alias porcino ID
                                          p.tagid,
                                          p.nombre,
                                          pc.ph_leptopirosis_fecha,
                                          pc.id AS leptopirosis_record_id, -- Alias leptopirosis record ID
                                          pc.ph_leptopirosis_producto,
                                          pc.ph_leptopirosis_dosis,
                                          pc.ph_leptopirosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN pc.ph_leptopirosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_leptopirosis_history
                                      FROM 
                                          porcino p
                                      LEFT JOIN 
                                          -- Join with all leptopirosis records per tagid
                                          ph_leptopirosis pc ON p.tagid = pc.ph_leptopirosis_tagid 
                                      ORDER BY 
                                          pc.ph_leptopirosis_fecha DESC, p.tagid ASC;";

                      $stmt = $conn->prepare($leptopirosisQuery);
                      $stmt->execute();
                      $leptopirosissData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($leptopirosissData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for leptopirosis records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT p_vencimiento_leptopirosis FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_leptopirosis'])) {
                                  $vigencia = intval($row_config['p_vencimiento_leptopirosis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($leptopirosissData as $row) {
                              echo "<tr>";
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-leptopirosis-btn" 
                              ...
                              if ($row['in_leptopirosis_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-leptopirosis" 
                                                  data-id="' . htmlspecialchars($row['leptopirosis_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['ph_leptopirosis_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ph_leptopirosis_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ph_leptopirosis_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ph_leptopirosis_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                                  echo '        <button class="btn btn-danger btn-sm delete-leptopirosis" 
                                                  data-id="' . htmlspecialchars($row['leptopirosis_record_id'] ?? '') . '"
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_leptopirosis_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_leptopirosis_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_leptopirosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_leptopirosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_leptopirosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_leptopirosis_fecha'])) {
                                          $leptopirosisDate = new DateTime($row['ph_leptopirosis_fecha']);
                                          $dueDate = clone $leptopirosisDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_leptopirosis_fecha']);
                      ...
                      error_log("Error in leptopirosis table: " . $e->getMessage());
...
<!-- Initialize DataTable for VH leptopirosis -->
<script>
$(document).ready(function() {
    $('#leptopirosisTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editLeptopirosisModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewLeptopirosis').click(function() {
        // Validate the form
        var form = document.getElementById('newLeptopirosisForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de leptopirosis ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de leptopirosis ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-leptopirosis').click(function() {
        var id = $(this).data('id');
        ...
        // edit modal HTML
        var modalHtml = `
        <div class="modal fade" id="editLeptopirosisModal" tabindex="-1" aria-labelledby="editLeptopirosisModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editLeptopirosisModalLabel">
                            <i class="fas fa-syringe me-2"></i>Editar Dosis de leptopirosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editLeptopirosisForm">
                            ...
                            <div class="mb-1">
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <select class="form-select" id="edit_vacuna" required>
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditLeptopirosis">
        ...
        $('#editLeptopirosisModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editLeptopirosisModal'));
        editModal.show();

        // Set the selected value in the dropdown after the modal is shown
        setTimeout(() => {
            const vacunaSelect = document.getElementById('edit_vacuna');
            if (vacunaSelect) {
                vacunaSelect.value = vacuna;
            }
        }, 100);

        // Handle save button click
        $('#saveEditLeptopirosis').click(function() {
            // Validate the form
            var form = document.getElementById('editLeptopirosisForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la dosis de leptopirosis ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            Swal.fire({
                                ...
                                text: 'El registro de leptopirosis ha sido actualizado correctamente',
                                icon: 'success',
                                confirmButtonColor: '#28a745'
                            }).then(() => {
                                // Reload the page to show updated data
    ...
    $('.delete-leptopirosis').click(function() {
        var id = $(this).data('id');
        var tagid = $(this).data('tagid');
    ...
    $(document).on('click', '.register-new-leptopirosis-btn', function() { 
...
<!-- Leptopirosis Line Chart Section -->

<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-success text-white">
            <h5 class="mb-0"><i class="fas fa-chart-line me-2"></i>Evolución Dosis Leptopirosis</h5>
        </div>
        <div class="card-body">
            <div class="row mb-2">
                <div class="col-md-4">
            ...
            <div class="chart-container" style="position: relative; height:50vh; width:100%">
                <canvas id="leptopirosisChart"></canvas>
            </div>
        </div>
    </div>
</div>

<!-- Chart.js Script for Leptopirosis Line Chart -->
<script>
$(document).ready(function() {
    let allLeptopirosisData = [];
    let leptopirosisChart = null;
    
    // Fetch leptopirosis data and create the chart
    $.ajax({
        url: 'get_leptopirosis_data.php',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            ...
            allLeptopirosisData = data;
            populateAnimalFilter(data);
            createLeptopirosisChart(data);
        ...
        error: function(xhr, status, error) {
            console.error('Error fetching leptopirosis data:', error);
        }
    });
    ...
    function updateChart() {
        ...
        let filteredData = [...allLeptopirosisData];
    ...
    function updateChartData(data) {
        if (leptopirosisChart) {
            leptopirosisChart.destroy();
        }
        createLeptopirosisChart(data);
    }
    
    function createLeptopirosisChart(data) {
        var ctx = document.getElementById('leptopirosisChart').getContext('2d');
        ...
        leptopirosisChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [{
                    ...
                    title: {
                        ...
                        text: function() {
                            ...
                            if (selectedAnimal !== 'all') {
                                ...
                                return 'Evolución de Dosis de Leptopirosis - ' + animalName;
                            }
                            return 'Evolución de Dosis de Leptopirosis - Todos los Animales';
                        },
                        font: {
                            size: 16
                        }
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Leptopirosis por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for Leptopirosis Status Pie Charts -->
<script>
$(document).ready(function() {
    const leptopirosisStatusUrl = 'get_leptopirosis_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="leptopirosisPie${safeGrupo}"></canvas>
                            <div id="leptopirosisPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        url: leptopirosisStatusUrl,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.error) {
                console.error('Error fetching leptopirosis status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `leptopirosisPie${safeGrupo}`;
                const messageId = `leptopirosisPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching leptopirosis status data:', error);
