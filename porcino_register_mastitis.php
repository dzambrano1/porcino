...
<title>Porcino - Registro de Vacunación Mastitis</title>
<!-- Link to the Favicon -->
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_mastitis_vacuna FROM pc_mastitis ORDER BY pc_mastitis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_mastitis_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_mastitis_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $mastitisQuery = "SELECT 
                                          ...
                                          p.tagid,
                                          p.nombre,
                                          pc.ph_mastitis_fecha,
                                          pc.id AS mastitis_record_id, -- Alias mastitis record ID
                                          pc.ph_mastitis_producto,
                                          pc.ph_mastitis_dosis,
                                          pc.ph_mastitis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN pc.ph_mastitis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_mastitis_history
                                      FROM 
                                          porcino p
                                      LEFT JOIN 
                                          -- Join with all mastitis records per tagid
                                          ph_mastitis pc ON p.tagid = pc.ph_mastitis_tagid 
                                      WHERE 
                                          p.grupo = 'Lactando'
                                      ORDER BY 
                                          pc.ph_mastitis_fecha DESC, p.tagid ASC;";
                      ...
                      if (empty($mastitisData)) {
                          ...
                          try {
                              $configQuery = "SELECT p_vencimiento_mastitis FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_mastitis'])) {
                                  $vigencia = intval($row_config['p_vencimiento_mastitis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($mastitisData as $row) {
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_mastitis_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_mastitis_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_mastitis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_mastitis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_mastitis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_mastitis_fecha'])) {
...
$(document).ready(function() {
    ...
    $.ajax({
        ...
        success: function(data) {
            console.log('Received mastitis data:', data); // Debug log
            
            if (data.error) {
                console.error('Server error:', data.error);
                $('#mastitisChart').parent().html('<div class="alert alert-danger">Error: ' + data.error + '</div>');
                return;
            }
            
            if (!data || data.length === 0) {
                console.log('No mastitis data available');
                $('#mastitisChart').parent().html('<div class="alert alert-info">No hay datos de mastitis disponibles para mostrar en el gráfico (solo se incluyen animales del grupo "Lactando").</div>');
                return;
            }
        ...
        error: function(xhr, status, error) {
            ...
            $('#mastitisChart').parent().html('<div class="alert alert-danger">Error al cargar datos: ' + error + '</div>');
        }
    });
    ...
    function createMastitisChart(data) {
        try {
            ...
            var dosis = data.map(function(item) {
                return parseFloat(item.dosis) || 0;
            });
            
            console.log('Mastitis chart labels:', labels);
            console.log('Mastitis chart data:', dosis);
        ...
        mastitisChart = new Chart(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    title: {
                        display: true,
                        text: 'Evolución de Dosis de Mastitis (Grupo Lactando)',
                        font: {
                            size: 16
                        }
                    }
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Mastitis - Grupo Lactando</h4>
...
<script>
$(document).ready(function() {
    const mastitisStatusUrl = 'get_mastitis_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="mastitisPie${safeGrupo}"></canvas>
                            <div id="mastitisPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `mastitisPie${safeGrupo}`;
                const messageId = `mastitisPie${safeGrupo}Message`;
