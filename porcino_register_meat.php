...
<title>Vacuno Peso Animal</title>
<!-- Link to the Favicon -->
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Animals and ALL their peso records (if any)
                        $pesoQuery = "
                            SELECT
                                ...
                                p.ph_peso_fecha,
                                p.ph_peso_tagid,         -- Matches porcino_tagid if peso exists
                                p.ph_peso_animal,
                                p.ph_peso_precio,
                                -- Calculate total_value only if p.id is not null
                                CASE WHEN p.id IS NOT NULL THEN CAST((p.ph_peso_animal * p.ph_peso_precio) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                porcino v
                            LEFT JOIN
                                ph_peso p ON v.tagid = p.ph_peso_tagid -- Join ALL matching peso records
                            ORDER BY
                      ...
                      if (empty($pesosData)) {
                          ...
                          try {
                              $configQuery = "SELECT p_vencimiento_pesaje_animal FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['p_vencimiento_pesaje_animal'])) {
                                  $vigencia = intval($row['p_vencimiento_pesaje_animal']);
                              }
                          ...
                          foreach ($pesosData as $row) {
                              ...
                              $pesoFecha = $row['ph_peso_fecha'] ?? null;
                              ...
                              if ($hasPeso) {
                                  ...
                                  echo '        <button class="btn btn-danger btn-sm delete-peso" 
                                                  data-id="'.htmlspecialchars($row['peso_id'] ?? '').'" 
                                                  data-tagid="'.htmlspecialchars($row['ph_peso_tagid'] ?? '').'" -- Pass tagid for context
                                                  title="Eliminar Peso">
                              ...
                              echo "<td>" . htmlspecialchars($row['porcino_tagid'] ?? 'N/A') . "</td>"; // Use porcino_tagid for consistency
                              // Column 5: Peso (kg)
                              echo "<td>" . ($hasPeso ? htmlspecialchars($row['ph_peso_animal'] ?? '') : 'N/A') . "</td>";
                              // Column 6: Precio ($/kg)
                              echo "<td>" . ($hasPeso ? htmlspecialchars($row['ph_peso_precio'] ?? '') : 'N/A') . "</td>";
