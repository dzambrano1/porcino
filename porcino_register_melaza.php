...
try {
    ...
    $melazaQuery = "SELECT 
                        ...
                        AND ph_melaza_racion IS NOT NULL 
                        AND ph_melaza_costo IS NOT NULL
    ...
    if (empty($melazaData)) {
        ...
        $melazaQuery = "SELECT 
                                ph_melaza_fecha,
                                ph_melaza_fecha as ph_melaza_fecha_inicio,
                                ph_melaza_fecha as ph_melaza_fecha_fin,
                                ph_melaza_racion,
                                ph_melaza_costo,
                                ph_melaza_tagid
                            FROM ph_melaza 
                            WHERE ph_melaza_fecha IS NOT NULL 
                            AND ph_melaza_racion IS NOT NULL 
                            AND ph_melaza_costo IS NOT NULL
                            ORDER BY ph_melaza_fecha ASC";
    ...
    foreach ($melazaData as $melaza) {
        $startDate = new DateTime($melaza['ph_melaza_fecha_inicio']);
        $endDate = new DateTime($melaza['ph_melaza_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $melaza['ph_melaza_racion'] * $melaza['ph_melaza_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing melaza: Start=" . $melaza['ph_melaza_fecha_inicio'] . 
                 ", End=" . $melaza['ph_melaza_fecha_fin'] . 
                 ", Days=" . $totalDays . 
                  ...
                  try {
                      // Query to get all Animals and ALL their melaza records (if any)
                        $melazaQuery = "
                            SELECT
                                ...
                                c.ph_melaza_fecha_inicio,
                                c.ph_melaza_fecha_fin,
                                c.ph_melaza_tagid,         -- Matches porcino_tagid if melaza exists
                                c.ph_melaza_etapa,
                                c.ph_melaza_producto,
                                c.ph_melaza_racion,
                                c.ph_melaza_costo,
                                -- Calculate total_value only if c.id is not null
                                CASE WHEN c.id IS NOT NULL THEN CAST((c.ph_melaza_racion * c.ph_melaza_costo) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                porcino v
                            LEFT JOIN
                                ph_melaza c ON v.tagid = c.ph_melaza_tagid -- Join ALL matching melaza records
                            ORDER BY
                      ...
                      if (empty($melazaData)) {
                          ...
                          foreach ($melazaData as $row) {
                              ...
                              $melazaFechaInicio = $row['ph_melaza_fecha_inicio'] ?? null;
                              ...
                              if ($hasMelaza) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-melaza" 
                                                  ...
                                                  data-debug="DB: '.htmlspecialchars($row['ph_melaza_producto'] ?? 'NULL').'">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['pc_melaza_nombre']) . '">' . htmlspecialchars($alimento_row['pc_melaza_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
