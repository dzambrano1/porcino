...
<title>Porcino - Registro de Neumonia</title>
<!-- Link to the Favicon -->
            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Neumonia</span>
...
<div class="container text-center">
  ...
  REGISTROS DE NEUMONIA
  </h3>
  
  <!-- New neumonia Entry Modal -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Neumonia
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newNeumoniaForm">
                <div class="mb-4">
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Neumonia</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_neumonia_vacuna FROM pc_neumonia ORDER BY pc_neumonia_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_neumonia_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_neumonia_vacuna']) . '</option>';
                                        }
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewNeumonia">
  ...
  <!-- DataTable for ph_neumonia records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="neumoniaTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      $neumoniaQuery = "SELECT 
                                          v.id AS porcino_id, -- Alias porcino ID
                                          v.tagid,
                                          v.nombre,
                                          va.ph_neumonia_fecha,
                                          va.id AS neumonia_record_id, -- Alias neumonia record ID
                                          va.ph_neumonia_producto,
                                          va.ph_neumonia_dosis,
                                          va.ph_neumonia_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_neumonia_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_neumonia_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all neumonia records per tagid
                                          ph_neumonia va ON v.tagid = va.ph_neumonia_tagid 
                                      ORDER BY 
                                          va.ph_neumonia_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($neumoniaQuery);
                      $stmt->execute();
                      $neumoniaData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($neumoniaData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for neumonia records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT p_vencimiento_neumonia FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_neumonia'])) {
                                  $vigencia = intval($row_config['p_vencimiento_neumonia']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($neumoniaData as $row) {
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-neumonia-btn" 
                              ...
                              if ($row['in_neumonia_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-neumonia" 
                                                  data-id="' . htmlspecialchars($row['neumonia_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['ph_neumonia_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ph_neumonia_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ph_neumonia_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ph_neumonia_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                                  echo '        <button class="btn btn-danger btn-sm delete-neumonia" 
                                                  data-id="' . htmlspecialchars($row['neumonia_record_id'] ?? '') . '"
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_neumonia_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_neumonia_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_neumonia_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_neumonia_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_neumonia_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_neumonia_fecha'])) {
                                          $neumoniaDate = new DateTime($row['ph_neumonia_fecha']);
                                          $dueDate = clone $neumoniaDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_neumonia_fecha']);
                      ...
                      error_log("Error in neumonia table: " . $e->getMessage());
...
<!-- Initialize DataTable for VH neumonia -->
<script>
$(document).ready(function() {
    $('#neumoniaTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editneumoniaModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewNeumonia').click(function() {
        // Validate the form
        var form = document.getElementById('newNeumoniaForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de neumonia ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de neumonia ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-neumonia').click(function() {
        ...
        // Edit neumonia Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editNeumoniaModal" tabindex="-1" aria-labelledby="editNeumoniaModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editNeumoniaModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Neumonia
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editNeumoniaForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Neumonia</label>
                    ...
                    <div class="modal-footer btn-group">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditNeumonia">
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Neumonia
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['pc_neumonia_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_neumonia_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        // Show the edit modal
        var editModal = new bootstrap.Modal(document.getElementById('editNeumoniaModal'));
        ...
        $('#saveEditNeumonia').click(function() {
            var formData = {
                id: $('#edit_id').val(),
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de neumonia para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
    ...
    $('.delete-neumonia').click(function() {
        var id = $(this).data('id');
        var tagid = $(this).data('tagid');
    ...
    $(document).on('click', '.register-new-neumonia-btn', function() { 
    ...
    <div class="card shadow-lg border-0">
        ...
        <div class="card-body p-4">
            ...
            <div class="chart-container">
                <canvas id="neumoniaChart"></canvas>
            </div>
        </div>
    </div>
</div>

<!-- Professional CSS Styling for Neumonia Charts -->
<style>
...
<!-- Chart.js Script for neumonia Line Chart -->
<script>
$(document).ready(function() {
    let allNeumoniaData = [];
    let neumoniaChart = null;
    
    // Add loading state to chart container
    $('.chart-container').addClass('loading');
    
    // Fetch neumonia data and create the chart
    $.ajax({
        url: 'get_neumonia_data.php',
        type: 'GET',
        dataType: 'json',
        beforeSend: function() {
            $('.chart-container').addClass('loading');
        ...
        success: function(data) {
            ...
            allNeumoniaData = data;
            populateAnimalFilter(data);
            createNeumoniaChart(data);
        ...
        error: function(xhr, status, error) {
            $('.chart-container').removeClass('loading');
            console.error('Error fetching neumonia data:', error);
        }
    });
    ...
    function updateChart() {
        ...
        let filteredData = [...allNeumoniaData];
    ...
    function updateChartData(data) {
        if (neumoniaChart) {
            neumoniaChart.destroy();
        }
        createNeumoniaChart(data);
    }
    
    function createNeumoniaChart(data) {
        var ctx = document.getElementById('neumoniaChart').getContext('2d');
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Neumonia por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for neumonia Status Pie Charts -->
<script>
$(document).ready(function() {
    const neumoniaStatusUrl = 'get_neumonia_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="neumoniaPie${safeGrupo}"></canvas>
                            <div id="neumoniaPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        url: neumoniaStatusUrl,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.error) {
                console.error('Error fetching neumonia status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `neumoniaPie${safeGrupo}`;
                const messageId = `neumoniaPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching neumonia status data:', error);
