...
<title>Porcino - Registro de Parasitos</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_parasitos_vacuna FROM pc_parasitos ORDER BY pc_parasitos_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_parasitos_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_parasitos_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for ph_parasitos records -->
              ...
              <tbody>
                  ...
                  try {
                      $parasitosQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.ph_parasitos_fecha,
                                          va.id AS parasitos_record_id, -- Alias parasitos record ID
                                          va.ph_parasitos_producto,
                                          va.ph_parasitos_dosis,
                                          va.ph_parasitos_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_parasitos_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_parasitos_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all parasitos records per tagid
                                          ph_parasitos va ON v.tagid = va.ph_parasitos_tagid 
                                      ORDER BY 
                                          va.ph_parasitos_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($parasitosQuery);
                      $stmt->execute();
                      $parasitosData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($parasitosData)) {
                          ...
                          try {
                              $configQuery = "SELECT p_vencimiento_parasitos FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_parasitos'])) {
                                  $vigencia = intval($row_config['p_vencimiento_parasitos']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($parasitosData as $row) {
                              ...
                              if ($row['in_parasitos_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-parasitos" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['ph_parasitos_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ph_parasitos_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ph_parasitos_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ph_parasitos_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_parasitos_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_parasitos_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_parasitos_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_parasitos_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_parasitos_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_parasitos_fecha'])) {
                                          $parasitosDate = new DateTime($row['ph_parasitos_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_parasitos_fecha']);
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Parasitos
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['pc_parasitos_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_parasitos_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
