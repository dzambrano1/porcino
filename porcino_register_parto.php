...
try {
    $chartQuery = "SELECT 
                    DATE_FORMAT(ph_parto_fecha, '%Y-%m') as month_year,
                    COUNT(*) as count,
                    GROUP_CONCAT(DISTINCT CONCAT(ph_parto_tagid, ':', ph_parto_numero)) as tagid_numeros
                  FROM ph_parto 
                  GROUP BY month_year 
...
<title>Porcino Register Parto</title>
<!-- Link to the Favicon -->
                  ...
                  try {
                      // Query to get all Female animals and their LATEST parto record (if any)
                        $partoQuery = "
                            ...
                            FROM
                                porcino p
                            LEFT JOIN ph_parto ins
                              ON p.tagid = ins.ph_parto_tagid
                            WHERE
                                p.genero = 'Hembra'
                            ORDER BY
                                p.nombre ASC,
                                ins.ph_parto_fecha DESC";
                      ...
                      if (empty($partoData)) {
                          ...
                          try {
                              $configQuery = "SELECT p_vencimiento_parto FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['p_vencimiento_parto'])) {
                                  $vigencia = intval($row['p_vencimiento_parto']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($partoData as $row) {
                              ...
                              $partoFecha = $row['ph_parto_fecha'] ?? null;
                              ...
                              echo "<td class='text-center'>" . ($hasparto ? htmlspecialchars($row['ph_parto_numero'] ?? '') : 'N/A') . "</td>";                              
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    gradient.addColorStop(0, 'rgba(133, 168, 50, 0.8)'); // Brighter blue at the top
    gradient.addColorStop(1, 'rgba(219, 232, 176, 0.5)'); // Fainter blue at the bottom
    ...
    new Chart(ctxParto, {
        ...
        data: {
            ...
            datasets: [{
                ...
                data: partoData.map(item => item.count),
        ...
        options: {
            ...
            plugins: {
                ...
                title: {
                    display: true,
                    text: 'Historial Mensual de Partos',
                ...
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            ...
                            const item = partoData[dataIndex];
                            const tooltipLines = [
                                `Cantidad: ${item.count}`,
                                'Animales:'
                            ];
                            ...
                            for (let i = 0; i < item.tagids.length; i++) {
                                tooltipLines.push(`TagID: ${item.tagids[i]} - Parto #${item.numeros[i]}`);
                            }
                ...
                y: {
                    ...
                    ticks: {
                        ...
                        font: {
                         ...
                         stepSize: Math.max(1, Math.ceil(Math.max(...partoData.map(item => item.count)) / 5)), // Dynamic step size (aim for ~5 steps)
