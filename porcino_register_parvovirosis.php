            ...
            <div class="row justify-content-center align-items-stretch">
                ...
                <div class="col-md-4 d-flex px-1 mb-3 mb-md-0">
                    <div class="arrow-step arrow-step-active w-100">
                        <span class="badge-active">🎯 Registrando Parvovirosis</span>
              ...
              <tbody>
                  ...
                  try {
                      $parvovirosisQuery = "SELECT 
                                          v.id AS porcino_id, -- Alias porcino ID
                                          v.tagid,
                                          v.nombre,
                                          va.ph_parvovirosis_fecha,
                                          va.id AS parvovirosis_record_id, -- Alias parvovirosis record ID
                                          va.ph_parvovirosis_producto,
                                          va.ph_parvovirosis_dosis,
                                          va.ph_parvovirosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.ph_parvovirosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_parvovirosis_history
                                      FROM 
                                          porcino v
                                      LEFT JOIN 
                                          -- Join with all parvovirosis records per tagid
                                          ph_parvovirosis va ON v.tagid = va.ph_parvovirosis_tagid 
                                      ORDER BY 
                                          va.ph_parvovirosis_fecha DESC, v.tagid ASC;";

                      $stmt = $conn->prepare($parvovirosisQuery);
                      $stmt->execute();
                      $parvovirosisData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($parvovirosisData)) {
                          ...
                          foreach ($parvovirosisData as $row) {
                              echo "<tr>";
...
$(document).ready(function() {
    ...
    $('.edit-parvovirosis').click(function() {
        ...
        // Edit parvovirosis Modal dialog for editing
        var modalHtml = `
        <div class="modal fade" id="editParvovirosisModal" tabindex="-1" aria-labelledby="editParvovirosisModalLabel">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editParvovirosisModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Vacunacion Parvovirosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editParvovirosisForm">
                            ...
                            <div class="mb-4">                        
                                <div class="input-group">
                                    <span class="input-group-text">
                                        <i class="fa-solid fa-syringe"></i>
                                        <label for="edit_vacuna" class="form-label">Vacunas Parvovirosis</label>
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="vacunaSelectionModalLabel">
                            <i class="fas fa-list me-2"></i>Seleccionar Vacuna de Parvovirosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['pc_parvovirosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_parvovirosis_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());
        ...
        $('#saveEditParvovirosis').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de parvovirosis para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
...
<!-- Professional CSS Styling for Parvovirosis Charts -->
<style>
