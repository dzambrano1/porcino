...
<title>Porcino - Registro de Vacunación Parvovirosis</title>
<!-- Link to the Favicon -->
...
<style>
#editParvovirosisModal #editParvovirosisForm > .mb-2 {
    display: flex;
    align-items: center;
...
<div class="container text-center">
  ...
  REGISTROS DE PARVOVIROSIS
  </h3>
  
  <!-- New Parvovirosis Entry Modal -->
        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newEntryModalLabel">
                    <i class="fas fa-plus-circle me-2"></i>Nueva Vacunacion Parvovirosis
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form id="newParvovirosisForm">
                <div class="mb-2">
                    ...
                    <div class="mb-2">                        
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <label for="new_vacuna" class="form-label">Vacunas Parvovirosis</label>
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    <option value="">Vacunas</option>
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT pc_parvovirosis_vacuna FROM pc_parvovirosis ORDER BY pc_parvovirosis_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['pc_parvovirosis_vacuna']) . '">' . htmlspecialchars($vacuna_row['pc_parvovirosis_vacuna']) . '</option>';
                                        }
            ...
            <div class="modal-footer btn-group">
                ...
                <button type="button" class="btn btn-success" id="saveNewParvovirosis">
  ...
  <!-- DataTable for ph_parvovirosis records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="parvovirosisTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      $parvovirosisQuery = "SELECT 
                                          p.id AS porcino_id, -- Alias porcino ID
                                          p.tagid,
                                          p.nombre,
                                          pc.ph_parvovirosis_fecha,
                                          pc.id AS parvovirosis_record_id, -- Alias parvovirosis record ID
                                          pc.ph_parvovirosis_producto,
                                          pc.ph_parvovirosis_dosis,
                                          pc.ph_parvovirosis_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN pc.ph_parvovirosis_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_parvovirosis_history
                                      FROM 
                                          porcino p
                                      LEFT JOIN 
                                          -- Join with all parvovirosis records per tagid
                                          ph_parvovirosis pc ON p.tagid = pc.ph_parvovirosis_tagid 
                                      ORDER BY 
                                          pc.ph_parvovirosis_fecha DESC, p.tagid ASC;";

                      $stmt = $conn->prepare($parvovirosisQuery);
                      $stmt->execute();
                      $parvovirosissData = $stmt->fetchAll(PDO::FETCH_ASSOC);

                      if (empty($parvovirosissData)) {
                          echo "<tr><td colspan='9' class='text-center'>No hay registros disponibles</td></tr>";
                      } else {
                          // Get vigencia setting for parvovirosis records
                          $vigencia = 30; // Default value
                          try {
                              $configQuery = "SELECT p_vencimiento_parvovirosis FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['p_vencimiento_parvovirosis'])) {
                                  $vigencia = intval($row_config['p_vencimiento_parvovirosis']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($parvovirosissData as $row) {
                              echo "<tr>";
                              ...
                              echo '        <button class="btn btn-success btn-sm register-new-parvovirosis-btn" 
                              ...
                              if ($row['in_parvovirosis_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-parvovirosis" 
                                                  data-id="' . htmlspecialchars($row['parvovirosis_record_id'] ?? '') . '" 
                                                  data-tagid="' . htmlspecialchars($row['tagid'] ?? '') . '" 
                                                  data-vacuna="' . htmlspecialchars($row['ph_parvovirosis_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['ph_parvovirosis_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['ph_parvovirosis_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['ph_parvovirosis_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                                                  <i class="fas fa-edit"></i>
                                              </button>';
                                  echo '        <button class="btn btn-danger btn-sm delete-parvovirosis" 
                                                  data-id="' . htmlspecialchars($row['parvovirosis_record_id'] ?? '') . '"
                              ...
                              echo "<td>" . htmlspecialchars($row['ph_parvovirosis_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_parvovirosis_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['ph_parvovirosis_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_parvovirosis_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['ph_parvovirosis_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['ph_parvovirosis_fecha'])) {
                                          $parvovirosisDate = new DateTime($row['ph_parvovirosis_fecha']);
                                          $dueDate = clone $parvovirosisDate;
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_parvovirosis_fecha']);
                      ...
                      error_log("Error in parvovirosis table: " . $e->getMessage());
...
<!-- Initialize DataTable for VH parvovirosis -->
<script>
$(document).ready(function() {
    $('#parvovirosisTable').DataTable({
...
$(document).ready(function() {
    ...
    // Note: editParvovirosisModal is created dynamically later, so no need to initialize here.

    // Handle new entry form submission
    $('#saveNewParvovirosis').click(function() {
        // Validate the form
        var form = document.getElementById('newParvovirosisForm');
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la dosis de parvovirosis ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        Swal.fire({
                            ...
                            text: 'El registro de parvovirosis ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-parvovirosis').click(function() {
        var id = $(this).data('id');
        ...
        // edit modal HTML
        var modalHtml = `
        <div class="modal fade" id="editParvovirosisModal" tabindex="-1" aria-labelledby="editParvovirosisModalLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editParvovirosisModalLabel">
                            <i class="fas fa-syringe me-2"></i>Editar Dosis de parvovirosis
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <form id="editParvovirosisForm">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="button" class="btn btn-success" id="saveEditParvovirosis">
        ...
        $('#editParvovirosisModal').remove();
        
        // Add the modal to the page
        $('body').append(modalHtml);
        
        // Show the modal
        var editModal = new bootstrap.Modal(document.getElementById('editParvovirosisModal'));
        editModal.show();
        ...
        $('#saveEditParvovirosis').click(function() {
            // Validate the form
            var form = document.getElementById('editParvovirosisForm');
            if (!form.checkValidity()) {
                form.reportValidity();
                return;
            }
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar la dosis de parvovirosis ${formData.dosis} ml para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            Swal.fire({
                                ...
                                text: 'El registro de parvovirosis ha sido actualizado correctamente',
                                icon: 'success',
                                confirmButtonColor: '#28a745'
                            }).then(() => {
                                // Reload the page to show updated data
    ...
    $('.delete-parvovirosis').click(function() {
        var id = $(this).data('id');
        var tagid = $(this).data('tagid');
    ...
    $(document).on('click', '.register-new-parvovirosis-btn', function() { 
...
<!-- Parvovirosis Line Chart Section -->

<div class="container mt-5 mb-5">
    <div class="card shadow">
        <div class="card-header bg-success text-white">
            <h5 class="mb-0"><i class="fas fa-chart-line me-2"></i>Evolución Dosis Parvovirosis</h5>
        </div>
        <div class="card-body">
            <div class="row mb-2">
                <div class="col-md-4">
            ...
            <div class="chart-container" style="position: relative; height:50vh; width:100%">
                <canvas id="parvovirosisChart"></canvas>
            </div>
        </div>
    </div>
</div>

<!-- Chart.js Script for Parvovirosis Line Chart -->
<script>
$(document).ready(function() {
    let allParvovirosisData = [];
    let parvovirosisChart = null;
    
    // Fetch parvovirosis data and create the chart
    $.ajax({
        url: 'get_parvovirosis_data.php',
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            ...
            allParvovirosisData = data;
            populateAnimalFilter(data);
            createParvovirosisChart(data);
        ...
        error: function(xhr, status, error) {
            console.error('Error fetching parvovirosis data:', error);
        }
    });
    ...
    function updateChart() {
        ...
        let filteredData = [...allParvovirosisData];
    ...
    function updateChartData(data) {
        if (parvovirosisChart) {
            parvovirosisChart.destroy();
        }
        createParvovirosisChart(data);
    }
    
    function createParvovirosisChart(data) {
        var ctx = document.getElementById('parvovirosisChart').getContext('2d');
        ...
        parvovirosisChart = new Chart(ctx, {
            type: 'line',
            data: {
                labels: labels,
                datasets: [{
                    ...
                    title: {
                        ...
                        text: function() {
                            ...
                            if (selectedAnimal !== 'all') {
                                ...
                                return 'Evolución de Dosis de Parvovirosis - ' + animalName;
                            }
                            return 'Evolución de Dosis de Parvovirosis - Todos los Animales';
                        },
                        font: {
                            size: 16
                        }
...
<!-- Pie Chart Section -->
<div class="container mt-5 mb-5">
    <h4 class="text-center text-black mb-4">Estado de Vacunación Parvovirosis por Grupos</h4>
    <div class="row justify-content-center" id="groupChartsContainer">
        <!-- Group charts will be populated dynamically here -->
    </div>
</div>

<!-- Chart.js Script for Parvovirosis Status Pie Charts -->
<script>
$(document).ready(function() {
    const parvovirosisStatusUrl = 'get_parvovirosis_status_by_grupo.php';
                ...
                <div class="card shadow">
                    ...
                    <div class="card-body">
                        <div class="chart-container" style="position: relative; height:40vh; width:100%">
                            <canvas id="parvovirosisPie${safeGrupo}"></canvas>
                            <div id="parvovirosisPie${safeGrupo}Message" class="text-center small text-muted mt-2"></div>
                        </div>
    ...
    $.ajax({
        url: parvovirosisStatusUrl,
        type: 'GET',
        dataType: 'json',
        success: function(data) {
            if (data.error) {
                console.error('Error fetching parvovirosis status data:', data.error);
            ...
            grupos.forEach(grupo => {
                ...
                const canvasId = `parvovirosisPie${safeGrupo}`;
                const messageId = `parvovirosisPie${safeGrupo}Message`;
        ...
        error: function(xhr, status, error) {
            console.error('AJAX error fetching parvovirosis status data:', error);
