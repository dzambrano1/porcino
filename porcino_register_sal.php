...
try {
    ...
    $salQuery = "SELECT 
                        ...
                        AND ph_sal_racion IS NOT NULL 
                        AND ph_sal_costo IS NOT NULL
                        ORDER BY ph_sal_fecha_inicio ASC";
    ...
    if (empty($salData)) {
        ...
        $salQuery = "SELECT 
                                ph_sal_fecha,
                                ph_sal_fecha as ph_sal_fecha_inicio,
                                ph_sal_fecha as ph_sal_fecha_fin,
                                ph_sal_racion,
                                ph_sal_costo,
                                ph_sal_tagid
                            FROM ph_sal 
                            WHERE ph_sal_fecha IS NOT NULL 
                            AND ph_sal_racion IS NOT NULL 
                            AND ph_sal_costo IS NOT NULL
                            ORDER BY ph_sal_fecha ASC";
    ...
    foreach ($salData as $sal) {
        $startDate = new DateTime($sal['ph_sal_fecha_inicio']);
        $endDate = new DateTime($sal['ph_sal_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $sal['ph_sal_racion'] * $sal['ph_sal_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing sal: Start=" . $sal['ph_sal_fecha_inicio'] . 
                 ", End=" . $sal['ph_sal_fecha_fin'] . 
                 ", Days=" . $totalDays . 
        ...
        <div class="table-responsive">
            ...
            try {
                      // Query to get all Animals and ALL their Sal records (if any)
                        $salQuery = "
                            ...
                            FROM
                                porcino v
                            LEFT JOIN
                                ph_sal c ON v.tagid = c.ph_sal_tagid -- Join ALL matching sal records
                            ORDER BY
                      ...
                      if (empty($salData)) {
                          ...
                          try {
                              $configQuery = "SELECT p_vencimiento_sal FROM p_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['p_vencimiento_sal'])) {
                                  $vigencia = intval($row['p_vencimiento_sal']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($salData as $row) {
                              ...
                              $salFechaInicio = $row['ph_sal_fecha_inicio'] ?? null;
                              ...
                              if ($hasSal) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-sal" 
                                                  ...
                                                  data-debug="DB: '.htmlspecialchars($row['ph_sal_producto'] ?? 'NULL').'">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['pc_sal_nombre']) . '">' . htmlspecialchars($alimento_row['pc_sal_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
