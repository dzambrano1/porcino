...
<div class="container text-center">
  ...
  <!-- DataTable for ph_inseminacion records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ph_inseminacionTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all ph_inseminacion records with animal name
                      $inseminacionQuery = "SELECT c.*, v.nombre as animal_nombre
                                FROM ph_inseminacion c 
                                LEFT JOIN porcino v ON c.ph_inseminacion_tagid = v.tagid 
                                ORDER BY c.ph_inseminacion_fecha DESC";
                      ...
                      if (empty($inseminacionData)) {
                          ...
                          foreach ($inseminacionData as $row) {
                              ...
                              try {
                                  $inseminacionDate = new DateTime($row['ph_inseminacion_fecha']);
                                  ...
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_inseminacion_fecha']);
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-inseminacion" 
                                          ...
                                          data-tagid="' . htmlspecialchars($row['ph_inseminacion_tagid'] ?? '') . '"
...
$(document).ready(function() {
    $('#ph_inseminacionTable').DataTable({
...
<div class="container text-center">
  ...
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ph_gestacionTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th class="text-center">ID</th>
                      <th class="text-center">Fecha</th>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all ph_gestacion records with animal name
                      $gestacionQuery = "SELECT c.*, v.nombre as animal_nombre
                                FROM ph_gestacion c 
                                LEFT JOIN porcino v ON c.ph_gestacion_tagid = v.tagid 
                                ORDER BY c.ph_gestacion_fecha DESC";
                      ...
                      if (empty($gestacionData)) {
                          ...
                          foreach ($gestacionData as $row) {
                              ...
                              // Calculate due date and determine status
                              try {
                                  $gestacionDate = new DateTime($row['ph_gestacion_fecha']);
                                  ...
                                  // Handle invalid date format
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_gestacion_fecha']);
                                  echo '<td class="text-center"><span class="badge bg-warning">ERROR</span></td>';
                              }
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-gestacion" 
                                          data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                          data-tagid="' . htmlspecialchars($row['ph_gestacion_tagid'] ?? '') . '"
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.ph_gestacion_tagid && !uniqueTagIds.has(item.ph_gestacion_tagid)) {
                uniqueTagIds.add(item.ph_gestacion_tagid);
                animals.push({
                    tagid: item.ph_gestacion_tagid,
                    nombre: item.animal_nombre || 'Sin nombre'
                });
            }
        });
    ...
    function updateChart() {
        ...
        // Filter by animal if not "all"
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.ph_gestacion_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.ph_gestacion_fecha) - new Date(b.ph_gestacion_fecha));
    ...
    function createGestacionChart(data) {
        ...
        var labels = data.map(function(item) {
            // Format the date for display
            var parts = item.ph_gestacion_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.ph_gestacion_fecha;
        });
        
        var numero = data.map(function(item) {
            return item.ph_gestacion_numero;
        });
...
<div class="container text-center">
  ...
  <!-- DataTable for ph_parto records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ph_partoTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th class="text-center">ID</th>
                      <th class="text-center">Fecha</th>
              ...
              <tbody>
                  ...
                  try {
                      ...
                      $partoQuery = "SELECT c.*, v.nombre as animal_nombre
                                FROM ph_parto c 
                                LEFT JOIN porcino v ON c.ph_parto_tagid = v.tagid 
                                ORDER BY c.ph_parto_fecha DESC";
                      ...
                      if (empty($partoData)) {
                          ...
                          foreach ($partoData as $row) {
                              ...
                              // Calculate due date and determine status
                              try {
                                  $partoDate = new DateTime($row['ph_parto_fecha']);
                                  ...
                                  // Handle invalid date format
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_parto_fecha']);
                                  echo '<td class="text-center"><span class="badge bg-warning">ERROR</span></td>';
                              }
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-parto" 
                                          data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                          data-tagid="' . htmlspecialchars($row['ph_parto_tagid'] ?? '') . '"
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.ph_parto_tagid && !uniqueTagIds.has(item.ph_parto_tagid)) {
                uniqueTagIds.add(item.ph_parto_tagid);
                animals.push({
                    tagid: item.ph_parto_tagid,
                    nombre: item.animal_nombre || 'Sin nombre'
                });
            }
        });
    ...
    function updateChart() {
        ...
        // Filter by animal if not "all"
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.ph_parto_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.ph_parto_fecha) - new Date(b.ph_parto_fecha));
    ...
    function createPartoChart(data) {
        ...
        var labels = data.map(function(item) {
            // Format the date for display
            var parts = item.ph_parto_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.ph_parto_fecha;
        });
        
        var numero = data.map(function(item) {
            return item.ph_parto_numero;
        });
...
<div class="container text-center">
  ...
  <!-- DataTable for ph_destete records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="ph_desteteTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th class="text-center">ID</th>
                      <th class="text-center">Fecha</th>
              ...
              <tbody>
                  ...
                  try {
                      ...
                      $desteteQuery = "SELECT c.*, v.nombre as animal_nombre
                                FROM ph_destete c 
                                LEFT JOIN porcino v ON c.ph_destete_tagid = v.tagid 
                                ORDER BY c.ph_destete_fecha DESC";
                      ...
                      if (empty($desteteData)) {
                          ...
                          foreach ($desteteData as $row) {
                              ...
                              // Calculate due date and determine status
                              try {
                                  $desteteDate = new DateTime($row['ph_destete_fecha']);
                                  ...
                                  // Handle invalid date format
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['ph_destete_fecha']);
                                  echo '<td class="text-center"><span class="badge bg-warning">ERROR</span></td>';
                              }
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-destete" 
                                          data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                          data-tagid="' . htmlspecialchars($row['ph_destete_tagid'] ?? '') . '"
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.ph_destete_tagid && !uniqueTagIds.has(item.ph_destete_tagid)) {
                uniqueTagIds.add(item.ph_destete_tagid);
                animals.push({
                    tagid: item.ph_destete_tagid,
                    nombre: item.animal_nombre || 'Sin nombre'
                });
            }
        });
    ...
    function updateChart() {
        ...
        // Filter by animal if not "all"
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.ph_destete_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.ph_destete_fecha) - new Date(b.ph_destete_fecha));
    ...
    function createDesteteChart(data) {
        ...
        var labels = data.map(function(item) {
            ...
            return item.ph_destete_fecha;
        });
        
        var peso = data.map(function(item) {
            return item.ph_destete_peso;
        });
