...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_peso WHERE ph_peso_tagid = ? ORDER BY ph_peso_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_peso ORDER BY ph_peso_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($pesoData as $row) {
        $date = new DateTime($row['ph_peso_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_peso_animal']);
        $pesoFechaLabels[] = $row['ph_peso_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_peso_precio']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_peso_animal']) * floatval($row['ph_peso_precio']);
...
<?php
    ...
    $pesoQuery = "SELECT p.*, 
                         ...
                         CAST((p.ph_peso_animal * p.ph_peso_precio) AS DECIMAL(10,2)) as total_value
                  FROM ph_peso p 
                  LEFT JOIN porcino v ON p.ph_peso_tagid = v.tagid 
                  ORDER BY p.ph_peso_fecha DESC";
...
foreach ($pesosResult as $row) {
    if ($row['ph_peso_tagid'] == '3000') {
        error_log("Debug - TagID 3000:");
        error_log("peso_animal: " . $row['ph_peso_animal']);
        error_log("peso_precio: " . $row['ph_peso_precio']);
        error_log("total_value: " . $row['total_value']);
    }
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_peso_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_peso_animal']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_peso_precio']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_peso_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-peso'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-peso='" . htmlspecialchars($row['ph_peso_animal']) . "'
                    data-precio='" . htmlspecialchars($row['ph_peso_precio']) . "'
                    data-fecha='" . htmlspecialchars($row['ph_peso_fecha']) . "'
