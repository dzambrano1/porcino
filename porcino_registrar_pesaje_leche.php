...
<div class="container scroll-icons-container">
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./porcino_registrar_pesaje_animal.php'" data-tooltip="Pesaje Carne">
    ...
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./porcino_registrar_alimentacion.php'" data-tooltip="Alimentacion">
    ...
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./porcino_registrar_reproduccion.php'" data-tooltip="Reproducion">
        <img src="./images/el-embarazo.png" alt="Reproduccion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" onclick="window.location.href='./porcino_registrar_poblacion.php'" data-tooltip="Poblacion">
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_leche WHERE ph_leche_tagid = ? ORDER BY ph_leche_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_leche ORDER BY ph_leche_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($lecheData as $row) {
        $date = new DateTime($row['ph_leche_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_leche_peso']);
        $lecheFechaLabels[] = $row['ph_leche_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_leche_precio']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_leche_peso']) * floatval($row['ph_leche_precio']);
...
<?php
    ...
    $lecheQuery = "SELECT p.*, 
                         ...
                         CAST((p.ph_leche_peso * p.ph_leche_precio) AS DECIMAL(10,2)) as total_value
                  FROM ph_leche p 
                  LEFT JOIN porcino v ON p.ph_leche_tagid = v.tagid 
                  ORDER BY p.ph_leche_fecha DESC";
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'porcino_registrar_leche.php';
                        });
                    } else {
...
foreach ($lechesResult as $row) {
    if ($row['ph_leche_tagid'] == '3000') {
        error_log("Debug - TagID 3000:");
        error_log("leche_animal: " . $row['ph_leche_peso']);
        error_log("leche_precio: " . $row['ph_leche_precio']);
        error_log("total_value: " . $row['total_value']);
    }
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_leche_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_leche_peso']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_leche_precio']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_leche_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-leche'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-leche='" . htmlspecialchars($row['ph_leche_peso']) . "'
                    data-precio='" . htmlspecialchars($row['ph_leche_precio']) . "'
                    data-fecha='" . htmlspecialchars($row['ph_leche_fecha']) . "'
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'porcino_historial.php';
                        });
                    } else {
