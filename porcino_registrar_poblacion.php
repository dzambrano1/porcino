...
<div class="container scroll-icons-container">
    ...
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./porcino_registrar_vacunacion.php'" data-tooltip="Salud">
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_compra WHERE ph_compra_tagid = ? ORDER BY ph_compra_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_compra ORDER BY ph_compra_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($compraData as $row) {
        $date = new DateTime($row['ph_compra_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_compra_peso']);
        $compraFechaLabels[] = $row['ph_compra_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_compra_precio']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_compra_peso']) * floatval($row['ph_compra_precio']);
...
<?php
    ...
    $compraQuery = "SELECT p.*, 
                         ...
                         CAST((p.ph_compra_peso * p.ph_compra_precio) AS DECIMAL(10,2)) as total_value
                  FROM ph_compra p 
                  LEFT JOIN porcino v ON p.ph_compra_tagid = v.tagid 
                  ORDER BY p.ph_compra_fecha DESC";
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'porcino_registrar_poblacion.php';
                        });
                    } else {
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_deceso WHERE ph_deceso_tagid = ? ORDER BY ph_deceso_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_deceso ORDER BY ph_deceso_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($decesoData as $row) {
        $date = new DateTime($row['ph_deceso_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_deceso_peso']);
        $decesoFechaLabels[] = $row['ph_deceso_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_deceso_peso']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_deceso_peso']) * floatval($row['ph_deceso_precio']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $decesoQuery = "SELECT p.*, 
                         v.nombre as animal_nombre                         
                  FROM ph_deceso p 
                  LEFT JOIN porcino v ON p.ph_deceso_tagid = v.tagid 
                  ORDER BY p.ph_deceso_fecha DESC";
...
foreach ($decesoResult as $row) {
    ...
    echo "<td class='text-center'>
            ...
            <button class='btn btn-success btn-sm edit-deceso'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-causa='" . htmlspecialchars($row['ph_deceso_causa']) . "'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_descarte WHERE ph_descarte_tagid = ? ORDER BY ph_descarte_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_descarte ORDER BY ph_descarte_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($descarteData as $row) {
        $date = new DateTime($row['ph_descarte_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_descarte_peso']);
        $descarteFechaLabels[] = $row['ph_descarte_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_descarte_peso']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_descarte_peso']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $descarteQuery = "SELECT p.*, 
                         v.nombre as animal_nombre                         
                  FROM ph_descarte p 
                  LEFT JOIN porcino v ON p.ph_descarte_tagid = v.tagid 
                  ORDER BY p.ph_descarte_fecha DESC";
...
foreach ($descarteResult as $row) {
    ...
    echo "<td class='text-center'>
            <button class='btn btn-success btn-sm edit-descarte'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-causa='" . htmlspecialchars($row['ph_descarte_peso']) . "'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_venta WHERE ph_venta_tagid = ? ORDER BY ph_venta_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_venta ORDER BY ph_venta_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($ventaData as $row) {
        $date = new DateTime($row['ph_venta_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_venta_peso']);
        $ventaFechaLabels[] = $row['ph_venta_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_venta_peso']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_venta_peso']) * floatval($row['ph_venta_precio']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $ventaQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.ph_venta_peso * p.ph_venta_precio) AS DECIMAL(10,2)) as total_value
                  FROM ph_venta p 
                  LEFT JOIN porcino v ON p.ph_venta_tagid = v.tagid 
                  ORDER BY p.ph_venta_fecha DESC";
