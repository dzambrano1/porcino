...
<div class="container scroll-icons-container">    
    ...
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./porcino_registrar_alimentacion.php'" data-tooltip="Salud">
    ...
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./porcino_registrar_poblacion.php'" data-tooltip="Poblacion">
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_inseminacion WHERE ph_inseminacion_tagid = ? ORDER BY ph_inseminacion_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_inseminacion ORDER BY ph_inseminacion_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($inseminacionData as $row) {
        $date = new DateTime($row['ph_inseminacion_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_inseminacion_costo']);
        $inseminacionFechaLabels[] = $row['ph_inseminacion_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_inseminacion_costo']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_inseminacion_costo']);
...
<?php
    ...
    $inseminacionQuery = "SELECT p.*, 
                         ...
                         p.ph_inseminacion_costo as total_value
                  FROM ph_inseminacion p 
                  LEFT JOIN porcino v ON p.ph_inseminacion_tagid = v.tagid 
                  ORDER BY p.ph_inseminacion_fecha DESC";
...
foreach ($inseminacionResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_inseminacion_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_inseminacion_numero']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_inseminacion_costo']) . "</td>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_inseminacion_fecha']) . "</td>";
    echo "<td class='text-center'>
            ...
            <button class='btn btn-success btn-sm edit-inseminacion'
                    ...
                    data-fecha='" . htmlspecialchars($row['ph_inseminacion_fecha']) . "'
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'porcino_registrar_reproduccion.php';
                        });
                    } else {
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_gestacion WHERE ph_gestacion_tagid = ? ORDER BY ph_gestacion_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_gestacion ORDER BY ph_gestacion_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($gestacionData as $row) {
        $date = new DateTime($row['ph_gestacion_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_gestacion_numero']);
        $gestacionFechaLabels[] = $row['ph_gestacion_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_gestacion_numero']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_gestacion_numero']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $gestacionQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         p.ph_gestacion_numero as total_value
                  FROM ph_gestacion p 
                  LEFT JOIN porcino v ON p.ph_gestacion_tagid = v.tagid 
                  ORDER BY p.ph_gestacion_fecha DESC";
...
foreach ($gestacionResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_gestacion_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_gestacion_numero']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_gestacion_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-gestacion'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-numero='" . htmlspecialchars($row['ph_gestacion_numero']) . "'
                    data-fecha='" . htmlspecialchars($row['ph_gestacion_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_parto WHERE ph_parto_tagid = ? ORDER BY ph_parto_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_parto ORDER BY ph_parto_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($partoData as $row) {
        $date = new DateTime($row['ph_parto_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_parto_numero']);
        $partoFechaLabels[] = $row['ph_parto_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_parto_numero']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_parto_numero']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $partoQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         p.ph_parto_numero as total_value
                  FROM ph_parto p 
                  LEFT JOIN porcino v ON p.ph_parto_tagid = v.tagid 
                  ORDER BY p.ph_parto_fecha DESC";
...
foreach ($partoResult as $row) {
    echo "<tr>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_parto_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_parto_numero']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_parto_fecha']) . "</td>";
    echo "<td class='text-center'>
            <button class='btn btn-success btn-sm edit-parto'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-numero='" . htmlspecialchars($row['ph_parto_numero']) . "'
                    data-fecha='" . htmlspecialchars($row['ph_parto_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_destete WHERE ph_destete_tagid = ? ORDER BY ph_destete_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_destete ORDER BY ph_destete_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($desteteData as $row) {
        $date = new DateTime($row['ph_destete_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['ph_destete_peso']);
        $desteteFechaLabels[] = $row['ph_destete_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['ph_destete_peso']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['ph_destete_peso']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $desteteQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.ph_destete_peso) AS DECIMAL(10,2)) as total_value
                  FROM ph_destete p 
                  LEFT JOIN porcino v ON p.ph_destete_tagid = v.tagid 
                  ORDER BY p.ph_destete_fecha DESC";
...
foreach ($desteteResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_destete_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_destete_peso']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_destete_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-destete'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-peso='" . htmlspecialchars($row['ph_destete_peso']) . "'
                    data-fecha='" . htmlspecialchars($row['ph_destete_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM ph_aborto WHERE ph_aborto_tagid = ? ORDER BY ph_aborto_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM ph_aborto ORDER BY ph_aborto_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($abortoData as $row) {
        $date = new DateTime($row['ph_aborto_fecha']);
        $monthKey = $date->format('Y-m');
        ...
        $abortoFechaLabels[] = $row['ph_aborto_fecha'];
    }
...
<?php
    ...
    $abortoQuery = "SELECT p.*, 
                  ...
                  FROM ph_aborto p 
                  LEFT JOIN porcino v ON p.ph_aborto_tagid = v.tagid 
                  ORDER BY p.ph_aborto_fecha DESC";
...
foreach ($abortoResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_aborto_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_aborto_causa']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['ph_aborto_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-aborto'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-causa='" . htmlspecialchars($row['ph_aborto_causa']) . "'
                    data-fecha='" . htmlspecialchars($row['ph_aborto_fecha']) . "'
                    data-bs-toggle='modal'
