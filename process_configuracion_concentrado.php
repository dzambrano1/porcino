...
try {
    ...
    switch ($action) {
        case 'insert':
            ...
            $requiredFields = ['concentrado', 'etapa', 'racion', 'costo', 'vigencia'];
            $missingFields = [];
            ...
            // Debug values
            error_log("Insert values: concentrado=$concentrado, etapa=$etapa, racion=$racion, costo=$costo, vigencia=$vigencia");

            // Insert new record - FIXED parameter names in SQL
            $sql = "INSERT INTO pc_concentrado (pc_concentrado_nombre, pc_concentrado_etapa, pc_concentrado_racion, pc_concentrado_costo, pc_concentrado_vigencia) 
                   VALUES (:concentrado, :etapa, :racion, :costo, :vigencia)";
        ...
        case 'update':
            // Validate required fields
            if (!isset($_POST['id']) || !isset($_POST['concentrado']) || !isset($_POST['etapa']) || 
            ...
            // Debug values
            error_log("Update values: id=$id, concentrado=$concentrado, etapa=$etapa, racion=$racion, costo=$costo, vigencia=$vigencia");

            // Update existing record
            $sql = "UPDATE pc_concentrado 
                    SET pc_concentrado_nombre = :concentrado, 
                        pc_concentrado_etapa = :etapa,
                        pc_concentrado_racion = :racion, 
                        pc_concentrado_costo = :costo, 
                        pc_concentrado_vigencia = :vigencia 
                    WHERE id = :id";
        ...
        case 'delete':
            ...
            // Delete record
            $sql = "DELETE FROM pc_concentrado WHERE id = :id";
