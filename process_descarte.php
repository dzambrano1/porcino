...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'update' && isset($_POST['tagid'], $_POST['peso'], $_POST['precio'], $_POST['fecha'])) {
        try {
            // Update porcino table with descarte information and set status to Descartado
            ...
            if ($stmt->rowCount() > 0) {
                $response = array(
                    ...
                    'redirect' => 'porcino_register_descarte.php'
                );
            } else {
                $response = array(
                    'success' => false,
        ...
        try {
            // Clear descarte information from porcino table and set status back to Activo
            $stmt = $conn->prepare("UPDATE porcino SET 
                descarte_fecha = NULL, 
                descarte_peso = NULL,
