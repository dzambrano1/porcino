...
if (isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'insert') {
            ...
            // Execute query
            $stmt = $conn->prepare("INSERT INTO ph_peso (ph_peso_tagid, ph_peso_animal, ph_peso_precio, ph_peso_fecha) VALUES (?, ?, ?, ?)");
        ...
        // UPDATE operation
            ...
            // Execute query
            $stmt = $conn->prepare("UPDATE ph_peso SET ph_peso_tagid = ?, ph_peso_animal = ?, ph_peso_precio = ?, ph_peso_fecha = ? WHERE id = ?");
        ...
        // DELETE operation
            ...
            // Execute query
            $stmt = $conn->prepare("DELETE FROM ph_peso WHERE id = ?");
            $stmt->execute([$id]);
