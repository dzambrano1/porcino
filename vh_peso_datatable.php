...
<div class="container mt-5">
        ...
        <table id="ph_pesoTable" class="table table-striped table-bordered">
            ...
            <tbody>
                ...
                try {
                    ...
                    $pesoQuery = "SELECT p.*, p.nombre as animal_nombre,
                                 CAST((pe.ph_peso_animal * pe.ph_peso_precio) AS DECIMAL(10,2)) as total_value
                                 FROM ph_peso pe 
                                 LEFT JOIN porcino p ON pe.ph_peso_tagid = p.tagid";
                                 
                    // Add search condition if provided
                    if (isset($_GET['search']) && !empty($_GET['search'])) {
                        $pesoQuery .= " WHERE p.ph_peso_tagid = :tagid";
                        $tagid = $_GET['search'];
                    }
                    
                    $pesoQuery .= " ORDER BY p.ph_peso_fecha DESC";
    ...
    <div class="mt-4 text-center">
        <a href="porcino_historial.php" class="btn btn-primary">Volver a Historial</a>
    </div>
</div>
